...
function buildStatBlock(poke) {
  ...
  return {
    ...
    sdef,
    spe,
    total: hp + atk + def + satk + sdef + spe,
  };
}
...
function scoreCandidateAgainstEnemy(candidate, enemy) {
  ...
  const attackUsed = enemyIsPhysicallyBulky
    ? candidate.stats.satk
    : candidate.stats.atk;
  const offensiveMultiplier = candidateTypes.length
    ? Math.max(
        0,
        ...candidateTypes.map((t) => typeEffectiveness(t, enemyTypes)),
      )
    : 1;
  ...
  const bulk =
    candidate.stats.hp +
    (enemyIsPhysicalAttacker ? candidate.stats.def : candidate.stats.sdef);
  ...
  return (
    autoTeamWeights.offense * scoreOff + autoTeamWeights.defense * scoreDef
  );
}
...
function averageCandidateScore(candidate, enemies) {
  ...
  for (const enemy of enemies)
    totalScore += scoreCandidateAgainstEnemy(candidate, enemy);
...
function buildGreedyTeam(enemies, area) {
  ...
  while (chosen.length < 6 && available.length > 0) {
    ...
    for (let i = 0; i < available.length; i++) {
      ...
      const overlap = normalizeTypes(candidate.ref?.type).filter((t) =>
        usedTypes.has(t),
      ).length;
...
function autoBuildTeam() {
  ...
  for (let i = 1; i <= 6; i++) {
    ...
    const lockThisItem =
      document.getElementById(`settings-auto-team-lock-item-${i}`)?.checked ===
      true;
