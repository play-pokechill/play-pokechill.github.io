...
function updateDictionary() {
  ...
  for (const i in ability) {
    ...
    if (
      ...
      !searchedDictionary.includes(ability[i])
    )
      continue;
  ...
  for (const i in item) {
    ...
    if (
      document.getElementById("dictionary-search").value != "" &&
      !searchedDictionary.includes(item[i])
    )
      continue;
  ...
  for (const i in move) {
    ...
    if (
      document.getElementById("dictionary-search").value != "" &&
      !searchedDictionary.includes(move[i])
    )
      continue;
  ...
  for (const i in pkmn) {
    ...
    if (
      document.getElementById("dictionary-search").value != "" &&
      !searchedDictionary.includes(pkmn[i])
    )
      continue;
...
function setSearchTags() {
  for (const e in pkmn) {
    ...
    if (
      wildlifePoolCommon.includes(e) ||
      wildlifePoolUncommon.includes(e) ||
      wildlifePoolRare.includes(e)
    )
      pkmn[e].tagObtainedIn = "park";
    if (exclusiveFrontierPkmn.includes(pkmn[e]))
  ...
  for (const e in pkmn) {
    if (pkmn[e].tagObtainedIn == undefined) {
      ...
      const familyHasDirectObtainable = Array.from(family).some((member) => {
        return (
          member.tagObtainedIn !== undefined &&
...
function getAbilityPoolByTier(poke) {
  ...
  Object.keys(ability).forEach((a) => {
    ...
    const matches =
      ab.type.includes("all") || ab.type.some((t) => types.includes(t));
  ...
  [1, 2, 3].forEach((rarity) => {
    ...
    const perAbilityChance = abilities.length
