...
function arrayPick(array, n = 1, seed) {
  ...
  const rng = seed === undefined ? Math.random : seed;
...
function givePkmn(poke, level) {
  ...
  const availableMoves = poke.movepool.filter(
    (m) => !equippedMoves.includes(m),
  );
...
function setWildPkmn() {
  ...
  if (saved.currentArea == areas.training.id) {
    ...
    document.getElementById(`training-indicator-counter`).textContent =
      `Remaining: ${currentTrainingWave}`;

    returnPkmnDivision(pkmn[saved.trainingPokemon]);

    let areaDivision = numericDivision(
      returnPkmnDivision(pkmn[saved.trainingPokemon]),
    );
    ...
    spawnedPkmn = randomDivisionPkmn(
      areaDivision,
      typeWeak(
        pkmn[saved.trainingPokemon].type[0],
        pkmn[saved.trainingPokemon].type[1],
        1,
      ),
      undefined,
      undefined,
    ...
    if (pkmn[saved.trainingPokemon].type[1] != undefined && rng(0.5))
      spawnedPkmn = randomDivisionPkmn(
        areaDivision,
        typeWeak(
          pkmn[saved.trainingPokemon].type[0],
    ...
    if (!pkmn[spawnedPkmn]) {
      ...
      spawnedPkmn = randomDivisionPkmn(
        areaDivision,
        typeWeak(pkmn[saved.trainingPokemon].type[0], 1),
        undefined,
        undefined,
        "training",
      );
      if (pkmn[saved.trainingPokemon].type[1] != undefined && rng(0.5))
        spawnedPkmn = randomDivisionPkmn(
          areaDivision,
          typeWeak(pkmn[saved.trainingPokemon].type[0], 2),
          undefined,
          undefined,
          "training",
        );
    ...
    if (areas.training.tier == 2)
      wildLevel = pkmn[saved.trainingPokemon].level + 10;
    if (areas.training.tier == 3)
      wildLevel = pkmn[saved.trainingPokemon].level + 20;
    ...
    for (let i = 0; i < 4; i++) {
      if (100 > i * 10) {
        ...
        if (moveKey != null) randomMoves.push(moveKey);
      }
    }

    if (currentTrainingWave <= 0) {
      training[areas.training.currentTraining].effect();
      leaveCombat();
      setTrainingMenu();
      return;
    }
    ...
    document.getElementById(`spiraling-highest-floor`).textContent =
      `Highest Floor: ${saved.maxSpiralFloor}`;
    document.getElementById(`spiraling-current-floor`).textContent =
    ...
    document.getElementById(`team-indicator-slot-1`).style.filter =
      `brightness(1)`;
    document.getElementById(`team-indicator-slot-2`).style.filter =
      `brightness(1)`;
    document.getElementById(`team-indicator-slot-3`).style.filter =
      `brightness(1)`;
    document.getElementById(`team-indicator-slot-4`).style.filter =
      `brightness(1)`;
    document.getElementById(`team-indicator-slot-5`).style.filter =
      `brightness(1)`;
    document.getElementById(`team-indicator-slot-6`).style.filter =
      `brightness(1)`;
    ...
    if (areas[saved.currentArea].team.slot2) {
      document.getElementById(`team-indicator-slot-2`).style.display = `flex`;
      maxTrainerSlot = 2;
    }
    if (areas[saved.currentArea].team.slot3) {
      document.getElementById(`team-indicator-slot-3`).style.display = `flex`;
      maxTrainerSlot = 3;
    }
    if (areas[saved.currentArea].team.slot4) {
      document.getElementById(`team-indicator-slot-4`).style.display = `flex`;
      maxTrainerSlot = 4;
    }
    if (areas[saved.currentArea].team.slot5) {
      document.getElementById(`team-indicator-slot-5`).style.display = `flex`;
      maxTrainerSlot = 5;
    }
    if (areas[saved.currentArea].team.slot6) {
      document.getElementById(`team-indicator-slot-6`).style.display = `flex`;
      maxTrainerSlot = 6;
    }

    if (currentTrainerSlot > 1)
      document.getElementById(`team-indicator-slot-1`).style.filter =
        `brightness(0.5)`;
    if (currentTrainerSlot > 2)
      document.getElementById(`team-indicator-slot-2`).style.filter =
        `brightness(0.5)`;
    if (currentTrainerSlot > 3)
      document.getElementById(`team-indicator-slot-3`).style.filter =
        `brightness(0.5)`;
    if (currentTrainerSlot > 4)
      document.getElementById(`team-indicator-slot-4`).style.filter =
        `brightness(0.5)`;
    if (currentTrainerSlot > 5)
      document.getElementById(`team-indicator-slot-5`).style.filter =
        `brightness(0.5)`;

    if (currentTrainerSlot > maxTrainerSlot) {
      //trainer won
        ...
        for (const i of rewards) {
          if (item[i.id] != undefined) {
            ...
            if (
              (item[i.id].type == "held" &&
                item[i.id].got >= 20 &&
                item[i.id].heldBonusPkmn == undefined) ||
              (item[i.id].heldBonusPkmn &&
                pkmn[item[i.id].heldBonusPkmn()].caught == 0 &&
                item[i.id].got >= 21) ||
              (item[i.id].heldBonusPkmn &&
                pkmn[item[i.id].heldBonusPkmn()].caught > 0 &&
                item[i.id].got >= 20)
      ...
      if (areas[saved.currentArea].encounterEffect)
        areas[saved.currentArea].encounterEffect();

      if (
        areas[saved.currentArea].encounter &&
        areas[saved.currentArea].unlockRequirement &&
        !areas[saved.currentArea].unlockRequirement()
      )
    ...
    spawnedPkmn = areas[saved.currentArea].team.slot1.id;
    randomMoves = areas[saved.currentArea].team.slot1Moves;
    if (areas[saved.currentArea].team.slot2 && currentTrainerSlot == 2) {
      spawnedPkmn = areas[saved.currentArea].team.slot2.id;
      randomMoves = areas[saved.currentArea].team.slot2Moves;
    }
    if (areas[saved.currentArea].team.slot3 && currentTrainerSlot == 3) {
      spawnedPkmn = areas[saved.currentArea].team.slot3.id;
      randomMoves = areas[saved.currentArea].team.slot3Moves;
    }
    if (areas[saved.currentArea].team.slot4 && currentTrainerSlot == 4) {
      spawnedPkmn = areas[saved.currentArea].team.slot4.id;
      randomMoves = areas[saved.currentArea].team.slot4Moves;
    }
    if (areas[saved.currentArea].team.slot5 && currentTrainerSlot == 5) {
      spawnedPkmn = areas[saved.currentArea].team.slot5.id;
      randomMoves = areas[saved.currentArea].team.slot5Moves;
    }
    if (areas[saved.currentArea].team.slot6 && currentTrainerSlot == 6) {
      spawnedPkmn = areas[saved.currentArea].team.slot6.id;
      randomMoves = areas[saved.currentArea].team.slot6Moves;
    }

    wildLevel = areas[saved.currentArea].level;
    hpMultiplier = 4;
  } else {
    if (areas[saved.currentArea].level !== undefined)
      wildLevel = random(
        areas[saved.currentArea].level - 9,
        areas[saved.currentArea].level,
      );

    if (areas[saved.currentArea].spawns.common)
      spawnedPkmn = arrayPick(areas[saved.currentArea].spawns.common).id;
    if (rng(0.08) && areas[saved.currentArea].spawns.uncommon)
      spawnedPkmn = arrayPick(areas[saved.currentArea].spawns.uncommon).id;
    if (rng(0.01) && areas[saved.currentArea].spawns.rare)
      spawnedPkmn = arrayPick(areas[saved.currentArea].spawns.rare).id;

    if (areas[saved.currentArea].spawns.common == undefined)
      spawnedPkmn = arrayPick(areas[saved.currentArea].spawns.rare).id;
  ...
  if (areas[saved.currentArea].difficulty != undefined)
    hpMultiplier = areas[saved.currentArea].difficulty;

  //abilities
  if (testAbility(`active`, ability.intimidate.id)) {
    wildBuffs.atkdown1 = 3;
    updateWildBuffs();
  }
  if (testAbility(`active`, ability.dauntingLook.id)) {
    wildBuffs.satkdown1 = 3;
    updateWildBuffs();
  }

  document.getElementById("explore-wild-name").innerHTML =
    format(spawnedPkmn) +
    ` <span class="explore-pkmn-level" > lvl ${wildLevel} </span>`;
  document.getElementById("explore-wild-sprite").src =
    `img/pkmn/sprite/${spawnedPkmn}.png`;

  if (pkmn[spawnedPkmn].float)
    document
      .getElementById("explore-wild-sprite")
      .classList.add(`floating-pkmn`);
  if (
    !pkmn[spawnedPkmn].float &&
    document
      .getElementById("explore-wild-sprite")
      .classList.contains(`floating-pkmn`)
  )
    document
      .getElementById("explore-wild-sprite")
      .classList.remove(`floating-pkmn`);
  ...
  if (saved.currentArea == areas.training.id)
    hpStars = returnDivisionStars(pkmn[spawnedPkmn]);
  if (
    saved.currentArea == areas.training.id &&
    pkmn[saved.trainingPokemon].type.includes("normal")
  )
    hpStars /= 1.5;
  if (
    saved.currentArea == areas.training.id &&
    pkmn[saved.trainingPokemon].type.includes("dragon")
  )
    hpStars /= 1.5;
  ...
  if (pkmn[saved.currentPkmn].temporalType != undefined)
    pkmn[saved.currentPkmn].temporalType = undefined;
  document.getElementById("explore-wild-sprite-data").dataset.pkmn =
    spawnedPkmn;
  ...
  randomMoves = randomMoves
    ...
    .concat(randomMoves.filter((m) => m === undefined));
  ...
  for (let index = 0; index < randomMoves.length; index++) {
    ...
    divMove.innerHTML = `<div id="pkmn-movebox-wild-${index + 1}-bar"
...
function updateItemsGot() {
  ...
  for (const i in item) {
    ...
    if (item[i].type !== "tm")
      divItem.innerHTML = `<img src="img/items/${i}.png"> <span>x${item[i].newItem}</span>`;
    if (item[i].type == "tm")
      divItem.innerHTML = `<img src="img/items/tm${format(move[item[i].move].type)}.png"> <span>x${item[i].newItem}</span>`;
...
function dropItem() {
  ...
  for (const slot in team) {
    ...
    if (team[slot].item == item.luckIncense.id)
      rareDropChance += item.luckIncense.power() / 100;
  }
  ...
  if (areas[saved.currentArea].drops?.uncommon && rng(0.15))
    drop = arrayPick(areas[saved.currentArea].drops?.uncommon).id;
  if (areas[saved.currentArea].drops?.rare && rng(rareDropChance))
    drop = arrayPick(areas[saved.currentArea].drops?.rare).id;

  if (item[drop].type == "held" && item[drop].got >= 20)
    drop = item.bottleCap.id;
  if (item[drop].type !== "held" && item[drop].evo && item[drop].got >= 10)
    drop = item.bottleCap.id;
...
function exitCombat() {
  ...
  afkSeconds = 0;
  storedAfkSeconds = 0;
  if (saved.tutorial && saved.tutorialStep === "battleEnd") {
...
function leaveCombat() {
  ...
  if (
    (areas[saved.currentArea].type == "vs" ||
      areas[saved.currentArea].type == "frontier") &&
    areas[saved.currentArea].defeated
  )
    document.getElementById("area-rejoin").style.display = "none";
  if (
    areas[saved.currentArea].id == areas.training.id &&
    training[areas.training.currentTraining].condition &&
    training[areas.training.currentTraining].condition() != true
  )
    document.getElementById("area-rejoin").style.display = "none";
  if (
    areas[saved.currentArea].unlockRequirement &&
  ...
  if (saved.tutorial && saved.tutorialStep === "battle") {
    saved.tutorialStep = "battleEnd";
    openTutorial();
  }
  ...
  for (const buff in wildBuffs) {
    if (wildBuffs[buff] > 0) wildBuffs[buff] = 0;
  }
  ...
  if (
    document
      ...
      .classList.contains(`menu-button-open`)
  )
    openMenu();
  ...
  if (
    ...
    saved.currentSpiralFloor == saved.maxSpiralFloor
  ) {
    ...
    if (saved.currentSpiralFloor > 29) {
      for (const slot in team) {
        ...
        pkmn[team[slot].pkmn.id].recordSpiraling = Math.max(
          saved.currentSpiralFloor,
          pkmn[team[slot].pkmn.id].recordSpiraling || 0,
        );
        console.log(
          Math.max(
            saved.currentSpiralFloor,
  ...
  for (const i in item) {
    ...
    divItem.dataset.item = i;
    if (item[i].type !== "tm")
      divItem.innerHTML = `<img src="img/items/${i}.png"><span>+${item[i].newItem}</span>`;
    if (item[i].type == "tm")
      divItem.innerHTML = `<img src="img/items/tm${format(move[item[i].move].type)}.png"><span>+${item[i].newItem}</span>`;
  ...
  for (const slot in team) {
    if (team[slot].pkmn === undefined) continue;
    if (team[slot].item == item.pureIncense.id)
      rarePkmnChance += item.pureIncense.power() / 100;
    if (team[slot].item == item.shinyCharm.id)
      shinyPkmnChance *= item.shinyCharm.power() / 100 + 1;
    if (team[slot].item == item.shinyCharm.id)
      shinyPkmnChanceEncounter *= item.shinyCharm.power() / 100 + 1;
  }
  ...
  if (item.mysteryEgg.got > 0 && areas[saved.currentArea].spawns != undefined) {
    ...
    for (let i = 0; i < item.mysteryEgg.got; i++) {
      ...
      if (areas[saved.currentArea].spawns.common)
        hatchedPkmn = arrayPick(areas[saved.currentArea].spawns.common).id;
      if (areas[saved.currentArea].spawns.uncommon && rng(0.08))
        hatchedPkmn = arrayPick(areas[saved.currentArea].spawns.uncommon).id;
      if (areas[saved.currentArea].spawns.rare && rng(rarePkmnChance))
        hatchedPkmn = arrayPick(areas[saved.currentArea].spawns.rare).id;
      ...
      if (rng(shinyPkmnChance) && pkmn[hatchedPkmn].shiny != true) {
        //shiny
      ...
      divPkmn.innerHTML =
        `<img class="sprite-trim" src="img/pkmn/sprite/${hatchedPkmn}.png">` +
        divTag;
      if (divTag == `<span>✦Shiny✦!</span>`)
        divPkmn.innerHTML =
          `<img class="sprite-trim" src="img/pkmn/shiny/${hatchedPkmn}.png">` +
          divTag;
  ...
  for (const i in pkmn) {
    // if new pokemon via trainer reward
    ...
    if (rng(shinyPkmnChanceEncounter)) {
      //shiny
      pkmn[i].shiny = true;
      divTag = `<span>✦Shiny✦!</span>`;
    }
    ...
    divPkmn.innerHTML =
      `<img class="sprite-trim" src="img/pkmn/sprite/${i}.png">` + divTag;
    if (divTag == `<span>✦Shiny✦!</span>`)
      divPkmn.innerHTML =
        `<img class="sprite-trim" src="img/pkmn/shiny/${i}.png">` + divTag;
    document.getElementById("area-end-pkmn-list").appendChild(divPkmn);
  ...
  if (noPkmn)
    document.getElementById("area-end-pkmn-title").style.display = "none";
  if (noPkmn)
    document.getElementById("area-end-pkmn-list").style.display = "none";
  if (noItems)
    document.getElementById("area-end-item-title").style.display = "none";
  if (noItems)
    document.getElementById("area-end-item-list").style.display = "none";
  if (noItems && noPkmn) {
    document.getElementById("area-end-item-title").innerHTML =
  ...
  if (
    item.autoRefightTicket.got > 0 &&
    areas[saved.currentArea].type != "vs" &&
    areas[saved.currentArea].type != "frontier" &&
    areas[saved.currentArea].id != "training" &&
    areas[saved.currentArea].encounter != true
  ) {
...
function rejoinArea() {
  ...
  if (storedAfkSeconds > 30) {
    //if the player is auto-refighting
...
function updateWildPkmn() {
  ...
  const hpBars = [
    ...
    {
      el: document.getElementById("exploe-wild-hp-2"),
      color: "rgb(134, 141, 238)",
    },
    {
      el: document.getElementById("exploe-wild-hp-3"),
      color: "rgb(238, 236, 134)",
    },
    {
      el: document.getElementById("exploe-wild-hp-4"),
  ...
  if (percent <= 0) {
    ...
    if (saved.currentArea == areas.frontierSpiralingTower.id) {
      saved.currentSpiralFloor++;
      saved.maxSpiralFloor = Math.max(
        saved.maxSpiralFloor,
        saved.currentSpiralFloor,
      );
      document.getElementById(`spiraling-current-floor`).textContent =
        `Floor: ${saved.currentSpiralFloor}`;
    ...
    //abilities
    if (testAbility(`active`, ability.moxie.id))
      moveBuff("wild", "atkup1", "self");
    if (testAbility(`active`, ability.strategist.id))
      moveBuff("wild", "satkup1", "self");
    if (testAbility(`active`, ability.speedBoost.id))
      moveBuff("wild", "speup1", "self");

    if (testAbility(`active`, ability.beastBoost.id)) {
      if (returnHighestStat(team[exploreActiveMember].pkmn) == "spe")
        moveBuff("wild", "speup1", "self");
      if (returnHighestStat(team[exploreActiveMember].pkmn) == "atk")
        moveBuff("wild", "atkup1", "self");
      if (returnHighestStat(team[exploreActiveMember].pkmn) == "def")
        moveBuff("wild", "defup1", "self");
      if (returnHighestStat(team[exploreActiveMember].pkmn) == "satk")
        moveBuff("wild", "satkup1", "self");
      if (returnHighestStat(team[exploreActiveMember].pkmn) == "sdef")
        moveBuff("wild", "sdefup1", "self");
    }
    ...
    if (
      rng(0.2) &&
      !areas[saved.currentArea]?.trainer &&
    ...
    voidAnimation(
      `explore-wild-sprite`,
      `wildPokemonDown ${respawnTimer + 1}s 1`,
    );
    ...
    let expGained = 0;
    if (wildLevel > pkmn[team[exploreActiveMember].pkmn.id].level - 10) {
      expGained = baseExpGain;
    }
    if (wildLevel >= pkmn[team[exploreActiveMember].pkmn.id].level + 5) {
      expGained = baseExpGain * 3;
    }
    if (wildLevel >= pkmn[team[exploreActiveMember].pkmn.id].level + 10) {
      expGained = baseExpGain * 6;
    }
    if (wildLevel >= pkmn[team[exploreActiveMember].pkmn.id].level + 20) {
      expGained = baseExpGain * 12;
    }
    if (wildLevel >= pkmn[team[exploreActiveMember].pkmn.id].level + 40) {
      expGained = baseExpGain * 64;
    }
    if (wildLevel >= pkmn[team[exploreActiveMember].pkmn.id].level + 50) {
      expGained = baseExpGain * 128;
    }
    if (pkmn[team[exploreActiveMember].pkmn.id].level == 100) expGained = 0;

    if (team[exploreActiveMember].item === "luckyEgg")
      expGained *= item.luckyEgg.power() / 100 + 1;
    ...
    for (const i in team) {
      ...
      expGained = 0;
      if (wildLevel > pkmn[team[i].pkmn.id].level - 10) {
        expGained = baseExpGain;
      }
      if (wildLevel >= pkmn[team[i].pkmn.id].level + 5) {
        expGained = baseExpGain * 3;
      }
      if (wildLevel >= pkmn[team[i].pkmn.id].level + 10) {
        expGained = baseExpGain * 6;
      }
      if (wildLevel >= pkmn[team[i].pkmn.id].level + 20) {
        expGained = baseExpGain * 12;
      }
      if (wildLevel >= pkmn[team[i].pkmn.id].level + 30) {
        expGained = baseExpGain * 32;
      }
      if (wildLevel >= pkmn[team[i].pkmn.id].level + 40) {
        expGained = baseExpGain * 64;
      }
      if (wildLevel >= pkmn[team[i].pkmn.id].level + 50) {
        expGained = baseExpGain * 128;
      }
      if (pkmn[team[i].pkmn.id].level == 100) expGained = 0;

      if (team[i].item === "luckyEgg")
        expGained *= item.luckyEgg.power() / 100 + 1;
...
function openMenu() {
  if (saved.firstTimePlaying) {
    if (
      pkmn.litten.caught == 0 &&
      pkmn.turtwig.caught == 0 &&
      pkmn.froakie.caught == 0
    ) {
  ...
  if (saved.mysteryGiftClaimed == true)
    document.getElementById(`menu-mystery-gift`).style.display = "none";
  const today = new Date();
  if (today > mysteryGift.duration)
    document.getElementById(`menu-mystery-gift`).style.display = "none";

  if (!saved.claimedExportReward) {
    document.getElementById(`menu-export-reward`).style.display = "flex";
  } else document.getElementById(`menu-export-reward`).style.display = "none";

  saved.currentAreaBuffer = undefined;

  if (areas.vsEliteFourLance.defeated == false) {
    document.getElementById(`menu-item-genetics`).className =
      `menu-item menu-item-locked`;
  } else {
    document.getElementById(`menu-item-genetics`).className = `menu-item`;
  }

  if (areas.vsGymLeaderMisty.defeated == false) {
    document.getElementById(`menu-item-training`).className =
      `menu-item menu-item-locked`;
  } else {
    document.getElementById(`menu-item-training`).className = `menu-item`;
  }

  if (areas.vsGymLeaderBrock.defeated == false) {
    document.getElementById(`menu-item-shop`).className =
      `menu-item menu-item-locked`;
    document.getElementById(`menu-export-reward`).className =
      `menu-item menu-item-locked`;
    document.getElementById(`menu-mystery-gift`).className =
      `menu-item menu-item-locked`;
  } else {
  ...
  if (saved.tutorialStep != "none") {
    document.getElementById(`menu-item-vs`).className =
      `menu-item menu-item-locked`;
    document.getElementById(`menu-item-items`).className =
      `menu-item menu-item-locked`;
    document.getElementById(`menu-item-team`).className =
      `menu-item menu-item-locked`;
    document.getElementById(`menu-item-dex`).className =
      `menu-item menu-item-locked`;
    document.getElementById(`menu-item-guide`).className =
      `menu-item menu-item-locked`;
    document.getElementById(`menu-item-dictionary`).className =
      `menu-item menu-item-locked`;
  } else {
  ...
  if (saved.currentArea !== undefined) {
    ...
    document.getElementById(`menu-item-training`).style.filter =
      "brightness(0.6)";
  } else {
  ...
  if (saved.currentArea == areas.training.id) {
    document.getElementById(`menu-item-travel`).style.filter =
      "brightness(0.6)";
    document.getElementById(`menu-item-training`).style.filter =
      "brightness(1)";
  } else
...
function updateTeamItems() {
  for (const slot in team) {
    if (team[slot].item === undefined) {
      if (document.getElementById(`team-${slot}-held-item`))
        document.getElementById(`team-${slot}-held-item`).innerHTML = "";
      continue;
    }

    const div = document.createElement("img");
    if (document.getElementById(`team-${slot}-held-item`))
      document.getElementById(`team-${slot}-held-item`).dataset.item =
        team[slot].item;
    div.src = `img/items/${team[slot].item}.png`;
    if (document.getElementById(`team-${slot}-held-item`))
      document.getElementById(`team-${slot}-held-item`).appendChild(div);
  }
}
...
function updateTeamExp() {
  for (const i in team) {
    ...
    if (pkmn[team[i].pkmn.id].exp >= 100) {
      // on level up

      if (pkmn[team[i].pkmn.id].level % 7 === 0) {
        //every 7 levels, learn move

        const learntMove = learnPkmnMove(
          pkmn[team[i].pkmn.id].id,
        ...
        if (learntMove !== undefined) {
          ...
          if (
            pkmn[team[i].pkmn.id].moves.slot2 === null ||
            pkmn[team[i].pkmn.id].moves.slot2 === undefined
          )
            pkmn[team[i].pkmn.id].moves.slot2 = learntMove; //fix
          else if (
            pkmn[team[i].pkmn.id].moves.slot3 === null ||
            pkmn[team[i].pkmn.id].moves.slot3 === undefined
          )
            pkmn[team[i].pkmn.id].moves.slot3 = learntMove;
          else if (
            pkmn[team[i].pkmn.id].moves.slot4 === null ||
            pkmn[team[i].pkmn.id].moves.slot4 === undefined
          )
            pkmn[team[i].pkmn.id].moves.slot4 = learntMove;
      ...
      pkmn[team[i].pkmn.id].level++;
      document.getElementById(`explore-${i}-lvl`).innerHTML =
        `lvl ${pkmn[team[i].pkmn.id].level}`;
      updateTeamExp();

      if (
        pkmn[team[i].pkmn.id].evolve &&
        pkmn[team[i].pkmn.id].evolve()[1].level > 0
      ) {
        // if it evolves by level up

        if (
          pkmn[team[i].pkmn.id].level >=
            pkmn[team[i].pkmn.id].evolve()[1].level &&
          pkmn[pkmn[team[i].pkmn.id].evolve()[1].pkmn.id].caught === 0
        ) {
...
function updateTeamPkmn() {
  for (const i in team) {
    ...
    const percent =
      (pkmn[team[i].pkmn.id].playerHp / pkmn[team[i].pkmn.id].playerHpMax) *
      100;
    if (percent > 60)
      document.getElementById(`explore-${i}-hp`).style.background =
        "rgb(130, 211, 130)";
    if (percent < 60)
      document.getElementById(`explore-${i}-hp`).style.background =
        "rgba(221, 168, 99, 1)";
    ...
    if (pkmn[team[i].pkmn.id].playerHp <= 0) {
      //on player death
      document.getElementById(`explore-${i}-hp`).style.width = "0%";
      document.getElementById(`explore-team-member-${i}-sprite`).style.filter =
        "grayscale(1)";
      document.getElementById(
        `explore-team-member-${i}-sprite`,
      ).style.animation = "none";
      document
        .getElementById(`explore-${i}-member`)
        .classList.add("member-inactive");
    }
    ...
    if (pkmn[team[exploreActiveMember].pkmn.id].playerHp <= 0) {
      ...
      document
        .getElementById(`explore-${exploreActiveMember}-member`)
        .classList.remove("member-inactive");
  ...
  if (
    saved.currentArea !== undefined &&
    (team?.slot6?.pkmn?.id === undefined ||
      pkmn[team.slot6.pkmn?.id].playerHp <= 0) &&
    (team?.slot5?.pkmn?.id === undefined ||
      pkmn[team.slot5.pkmn?.id].playerHp <= 0) &&
    (team?.slot4?.pkmn?.id === undefined ||
      pkmn[team.slot4.pkmn?.id].playerHp <= 0) &&
    (team?.slot3?.pkmn?.id === undefined ||
      pkmn[team.slot3.pkmn?.id].playerHp <= 0) &&
    (team?.slot2?.pkmn?.id === undefined ||
      pkmn[team.slot2.pkmn?.id].playerHp <= 0) &&
    (team?.slot1?.pkmn?.id === undefined ||
      pkmn[team.slot1.pkmn?.id].playerHp <= 0)
  ) {
    leaveCombat();

    if (saved.autoRefight == true) {
      if (areas[saved.currentArea].encounter != true)
        item.autoRefightTicket.got--;
      if (
        areas[saved.currentArea].encounter != true &&
        item.autoRefightTicket.got < 1
      )
        saved.autoRefight = false;
    }
  }
...
function returnPkmnTypes(id) {
  if (pkmn[id].type[1] != undefined)
    return `<div style="background:${returnTypeColor(pkmn[id].type[0])}">${format(pkmn[id].type[0])}</div><div style="background:${returnTypeColor(pkmn[id].type[1])}">${format(pkmn[id].type[1])}</div>`;
  else
    return `<div style="background:${returnTypeColor(pkmn[id].type[0])}">${format(pkmn[id].type[0])}</div>`;
}
...
function shouldCombatStop() {
  if (document.getElementById(`tooltipBackground`).style.display === "flex")
    return true;
  ...
  if (document.getElementById(`team-menu`).style.display === "flex")
    return true;
...
function exploreCombatPlayer() {
  ...
  if (
    nextMoveBoxPlayer !=
    document.getElementById(
      `pkmn-movebox-slot${exploreCombatPlayerTurn}-team-${exploreActiveMember}`,
    )
  )
    nextMoveBoxPlayer = document.getElementById(
      `pkmn-movebox-slot${exploreCombatPlayerTurn}-team-${exploreActiveMember}`,
    );
  if (nextMovePlayer != nextMoveBoxPlayer?.dataset?.move)
    nextMovePlayer = nextMoveBoxPlayer?.dataset?.move;
  if (moveTimerPlayer != move[nextMovePlayer]?.timer)
    moveTimerPlayer = move[nextMovePlayer]?.timer;
  if (
    barPlayer !=
    document.getElementById(
      `pkmn-movebox-slot${exploreCombatPlayerTurn}-team-${exploreActiveMember}-bar`,
    )
  )
    barPlayer = document.getElementById(
      `pkmn-movebox-slot${exploreCombatPlayerTurn}-team-${exploreActiveMember}-bar`,
    );
  ...
  //abilities
  if (
    testAbility(`active`, ability.prankster.id) &&
    (move[nextMovePlayer].type == "dark" ||
      move[nextMovePlayer].type == "ghost")
  )
    moveTimerPlayer /= 1.5;
  if (
    testAbility(`active`, ability.galeWings.id) &&
    (move[nextMovePlayer].type == "flying" ||
      move[nextMovePlayer].type == "bug")
  )
    moveTimerPlayer /= 1.5;

  if (
    testAbility(`active`, ability.prankster.id) &&
    testAbility(`active`, ability.gloomilate.id) &&
    move[nextMovePlayer].type == "normal"
  )
    moveTimerPlayer /= 1.5;
  if (
    testAbility(`active`, ability.galeWings.id) &&
    testAbility(`active`, ability.aerilate.id) &&
    move[nextMovePlayer].type == "normal"
  )
    moveTimerPlayer /= 1.5;
  if (
    testAbility(`active`, ability.galeWings.id) &&
    testAbility(`active`, ability.chrysilate.id) &&
    move[nextMovePlayer].type == "normal"
  )
    moveTimerPlayer /= 1.5;

  if (
    testAbility(`active`, ability.cacophony.id) &&
    move[nextMovePlayer].affectedBy?.includes(ability.cacophony.id)
  )
    moveTimerPlayer /= 2;
  if (
    testAbility(`active`, ability.dancer.id) &&
    move[nextMovePlayer].affectedBy?.includes(ability.dancer.id)
  )
    moveTimerPlayer /= 2;
  ...
  if (areas[saved.currentArea].id == areas.training.id)
    speedStars = returnDivisionStars(pkmn[team[exploreActiveMember].pkmn.id]);

  barProgressPlayer +=
    100 /
    ((moveTimerPlayer *
      (Math.pow(0.9, speedStars) *
        Math.pow(0.95, pkmn[team[exploreActiveMember].pkmn.id].ivs.spe))) /
      (1000 / 60));
  ...
  if (barProgressPlayer < 99) {
    ...
    if (nextMove.id == move.meFirst.id)
      nextMove =
        move[document?.getElementById(`pkmn-movebox-wild-1`).dataset.move];
    ...
    //abilities
    if (testAbility(`active`, ability.technician.id) && movePower <= 60)
      movePower *= 1.5;

    let multihit = 1;
    if (nextMove.multihit)
      multihit = random(nextMove.multihit[0], nextMove.multihit[1]);
    if (nextMove.multihit && testAbility(`active`, ability.skillLink.id))
      multihit = nextMove.multihit[1];
    movePower *= multihit;
    ...
    if (
      testAbility(`active`, ability.ironFist.id) &&
      nextMove.affectedBy?.includes(ability.ironFist.id)
    )
      movePower *= 1.5;
    if (
      testAbility(`active`, ability.strongJaw.id) &&
      nextMove.affectedBy?.includes(ability.strongJaw.id)
    )
      movePower *= 2;
    if (
      testAbility(`active`, ability.toughClaws.id) &&
      nextMove.affectedBy?.includes(ability.toughClaws.id)
    )
      movePower *= 2;
    if (
      testAbility(`active`, ability.sharpness.id) &&
      nextMove.affectedBy?.includes(ability.sharpness.id)
    )
      movePower *= 1.5;
    if (
      testAbility(`active`, ability.megaLauncher.id) &&
      nextMove.affectedBy?.includes(ability.megaLauncher.id)
    )
      movePower *= 1.5;
    if (
      testAbility(`active`, ability.metalhead.id) &&
      nextMove.affectedBy?.includes(ability.metalhead.id)
    )
      movePower *= 1.5;

    if (
      testAbility(`active`, ability.reckless.id) &&
      nextMove.timer > defaultPlayerMoveTimer
    )
      movePower *= 1.5;
    if (
      testAbility(`active`, ability.libero.id) &&
      nextMove.timer < defaultPlayerMoveTimer
    )
      movePower *= 2;
    ...
    if (nextMove.split == "physical") {
      totalPower =
        (movePower +
          Math.max(
            0,
            attacker.bst.atk * 30 * Math.pow(1.1, attacker.ivs.atk) -
              defender.bst.def * 30,
          )) *
      ...
      if (areas[saved.currentArea].id == areas.training.id) {
        totalPower =
          (movePower +
            Math.max(
              0,
              returnDivisionStars(attacker, "atk") *
                30 *
                Math.pow(1.1, attacker.ivs.atk) -
                returnDivisionStars(defender) * 30,
            )) *
          (1 + attacker.level * 0.1) *
          1;
      }
    ...
    if (nextMove.split == "special") {
      totalPower =
        (movePower +
          Math.max(
            0,
            attacker.bst.satk * 30 * Math.pow(1.1, attacker.ivs.satk) -
              defender.bst.sdef * 30,
          )) *
        (1 + attacker.level * 0.1) *
        1;
      ...
      if (areas[saved.currentArea].id == areas.training.id) {
        totalPower =
          (movePower +
            Math.max(
              0,
              returnDivisionStars(attacker, "satk") *
                30 *
                Math.pow(1.1, attacker.ivs.atk) -
                returnDivisionStars(defender) * 30,
            )) *
    ...
    if (nextMove.split == "special") {
      ...
      if (
        team[exploreActiveMember].buffs?.poisoned > 0 &&
        !testAbility(`active`, ability.guts.id)
      )
        totalPower /= 1.5;
    ...
    if (nextMove.split == "physical") {
      ...
      if (
        team[exploreActiveMember].buffs?.burn > 0 &&
        !testAbility(`active`, ability.guts.id)
      )
        totalPower /= 1.5;
    ...
    if (testAbility(`active`, ability.normalize.id)) {
      moveType = "normal";
      totalPower *= 1.2;
    }

    if (testAbility(`active`, ability.ferrilate.id) && moveType == "normal") {
      moveType = "steel";
      totalPower *= 1.3;
    }
    if (testAbility(`active`, ability.glaciate.id) && moveType == "normal") {
      moveType = "ice";
      totalPower *= 1.3;
    }
    if (testAbility(`active`, ability.terralate.id) && moveType == "normal") {
      moveType = "ground";
      totalPower *= 1.3;
    }
    if (testAbility(`active`, ability.toxilate.id) && moveType == "normal") {
      moveType = "poison";
      totalPower *= 1.3;
    }
    if (testAbility(`active`, ability.hydrolate.id) && moveType == "normal") {
      moveType = "water";
      totalPower *= 1.3;
    }
    if (testAbility(`active`, ability.pyrolate.id) && moveType == "normal") {
      moveType = "fire";
      totalPower *= 1.3;
    }
    if (testAbility(`active`, ability.chrysilate.id) && moveType == "normal") {
      moveType = "bug";
      totalPower *= 1.3;
    }
    if (testAbility(`active`, ability.galvanize.id) && moveType == "normal") {
      moveType = "electric";
      totalPower *= 1.3;
    }
    if (testAbility(`active`, ability.gloomilate.id) && moveType == "normal") {
      moveType = "dark";
      totalPower *= 1.3;
    }
    if (testAbility(`active`, ability.espilate.id) && moveType == "normal") {
      moveType = "psychic";
      totalPower *= 1.3;
    }
    if (testAbility(`active`, ability.aerilate.id) && moveType == "normal") {
      moveType = "flying";
      totalPower *= 1.3;
    }
    if (testAbility(`active`, ability.pixilate.id) && moveType == "normal") {
      moveType = "fairy";
      totalPower *= 1.3;
    }
    if (testAbility(`active`, ability.verdify.id) && moveType == "normal") {
      moveType = "grass";
      totalPower *= 1.3;
    }

    if (testAbility(`active`, ability.protean.id))
      pkmn[team[exploreActiveMember].pkmn.id].type = [moveType];

    //stab
    let stabBonus = 1.5;
    if (
      attacker.type.includes(moveType) &&
      testAbility(`active`, ability.adaptability.id)
    )
      stabBonus = 1.7;

    if (attacker.type.includes(moveType)) totalPower *= stabBonus;
    //type effectiveness
    let typeMultiplier = typeEffectiveness(
      moveType,
      pkmn[saved.currentPkmn].type,
    );

    if (
      testAbility(`active`, ability.scrappy.id) &&
      pkmn[saved.currentPkmn].type.includes("ghost") &&
      (moveType == "fighting" || moveType == "normal")
    )
      typeMultiplier = 1;
    if (
      testAbility(`active`, ability.tintedLens.id) &&
      (typeMultiplier == 0.5 || typeMultiplier == 0.25)
    )
      typeMultiplier = 1;
    if (testAbility(`active`, ability.noGuard.id) && typeMultiplier == 0)
      typeMultiplier = 1;

    if (defender.temporalType)
      typeMultiplier *= typeEffectiveness(moveType, defender.temporalType);

    //this wont exactly do what the move says it does, but eh, close enough
    if (
      nextMove.id == move.freezeDry.id &&
      pkmn[saved.currentPkmn].type == "water" &&
      typeEffectiveness("water", pkmn[saved.currentPkmn].type) == 1
    )
      typeMultiplier += 0.5;
    ...
    //items
    if (
      team[exploreActiveMember].item == item.blackBelt.id &&
      moveType == "fighting"
    )
      totalPower *= item.blackBelt.power() / 100 + 1;
    if (
      team[exploreActiveMember].item == item.blackGlasses.id &&
      moveType == "dark"
    )
      totalPower *= item.blackGlasses.power() / 100 + 1;
    if (
      team[exploreActiveMember].item == item.charcoal.id &&
      moveType == "fire"
    )
      totalPower *= item.charcoal.power() / 100 + 1;
    if (
      team[exploreActiveMember].item == item.dragonFang.id &&
      moveType == "dragon"
    )
      totalPower *= item.dragonFang.power() / 100 + 1;
    if (
      team[exploreActiveMember].item == item.fairyFeather.id &&
      moveType == "fairy"
    )
      totalPower *= item.fairyFeather.power() / 100 + 1;
    if (
      team[exploreActiveMember].item == item.hardStone.id &&
      moveType == "rock"
    )
      totalPower *= item.hardStone.power() / 100 + 1;
    if (
      team[exploreActiveMember].item == item.magnet.id &&
      moveType == "electric"
    )
      totalPower *= item.magnet.power() / 100 + 1;
    if (
      team[exploreActiveMember].item == item.metalCoat.id &&
      moveType == "steel"
    )
      totalPower *= item.metalCoat.power() / 100 + 1;
    if (
      team[exploreActiveMember].item == item.miracleSeed.id &&
      moveType == "grass"
    )
      totalPower *= item.miracleSeed.power() / 100 + 1;
    if (
      team[exploreActiveMember].item == item.mysticWater.id &&
      moveType == "water"
    )
      totalPower *= item.mysticWater.power() / 100 + 1;
    if (
      team[exploreActiveMember].item == item.neverMeltIce.id &&
      moveType == "ice"
    )
      totalPower *= item.neverMeltIce.power() / 100 + 1;
    if (
      team[exploreActiveMember].item == item.poisonBarb.id &&
      moveType == "poison"
    )
      totalPower *= item.poisonBarb.power() / 100 + 1;
    if (
      team[exploreActiveMember].item == item.sharpBeak.id &&
      moveType == "flying"
    )
      totalPower *= item.sharpBeak.power() / 100 + 1;
    if (
      team[exploreActiveMember].item == item.silkScarf.id &&
      moveType == "normal"
    )
      totalPower *= item.silkScarf.power() / 100 + 1;
    if (
      team[exploreActiveMember].item == item.silverPowder.id &&
      moveType == "bug"
    )
      totalPower *= item.silverPowder.power() / 100 + 1;
    if (
      team[exploreActiveMember].item == item.softSand.id &&
      moveType == "ground"
    )
      totalPower *= item.softSand.power() / 100 + 1;
    if (
      team[exploreActiveMember].item == item.spellTag.id &&
      moveType == "ghost"
    )
      totalPower *= item.spellTag.power() / 100 + 1;
    if (
      team[exploreActiveMember].item == item.twistedSpoon.id &&
      moveType == "psychic"
    )
      totalPower *= item.twistedSpoon.power() / 100 + 1;

    if (team[exploreActiveMember].item == item.lightClay.id)
      totalPower *= item.lightClay.power() / 100 + 1;

    if (team[exploreActiveMember].item == item.flameOrb.id) {
      totalPower *= item.flameOrb.power();
      team[exploreActiveMember].buffs.burn = 3;
    }
    if (team[exploreActiveMember].item == item.toxicOrb.id) {
      totalPower *= item.toxicOrb.power();
      team[exploreActiveMember].buffs.poisoned = 3;
    }

    if (
      team[exploreActiveMember].item == item.choiceSpecs.id &&
      nextMove.split == "special"
    )
      totalPower *= item.choiceSpecs.power();
    if (
      team[exploreActiveMember].item == item.choiceBand.id &&
      nextMove.split == "physical"
    )
      totalPower *= item.choiceBand.power();

    if (team[exploreActiveMember].item == item.lifeOrb.id)
      totalPower *= item.lifeOrb.power();

    //held items w specific damage bonus
    if (
      item[team[exploreActiveMember].item]?.heldBonusPkmn &&
      item[team[exploreActiveMember].item].heldBonusPkmn() ==
        team[exploreActiveMember].pkmn.id
    )
      totalPower *= item[team[exploreActiveMember].item].heldBonusPower();

    if (nextMove.power === 0) totalPower = 0;

    if (areas[saved.currentArea].id != areas.training.id) {
      if (team[exploreActiveMember].buffs?.confused > 0 && rng(0.5))
        totalPower = 0;
      if (team[exploreActiveMember].buffs?.paralysis > 0 && rng(0.25))
        totalPower = 0;
    ...
    //abilities
    const below50hp =
      pkmn[team[exploreActiveMember].pkmn.id]?.playerHp <
      pkmn[team[exploreActiveMember].pkmn.id]?.playerHpMax * 0.5;
    if (
      below50hp &&
      testAbility(`active`, ability.overgrow.id) &&
      moveType == "grass"
    )
      totalPower *= 1.3;
    if (
      below50hp &&
      testAbility(`active`, ability.blaze.id) &&
      moveType == "fire"
    )
      totalPower *= 1.3;
    if (
      below50hp &&
      testAbility(`active`, ability.swarm.id) &&
      moveType == "bug"
    )
      totalPower *= 1.3;
    if (
      below50hp &&
      testAbility(`active`, ability.torrent.id) &&
      moveType == "water"
    )
      totalPower *= 1.3;
    if (
      below50hp &&
      testAbility(`active`, ability.bastion.id) &&
      moveType == "steel"
    )
      totalPower *= 1.3;
    if (
      below50hp &&
      testAbility(`active`, ability.average.id) &&
      moveType == "normal"
    )
      totalPower *= 1.3;
    if (
      below50hp &&
      testAbility(`active`, ability.resolve.id) &&
      moveType == "fighting"
    )
      totalPower *= 1.3;
    if (
      below50hp &&
      testAbility(`active`, ability.mistify.id) &&
      moveType == "psychic"
    )
      totalPower *= 1.3;
    if (
      below50hp &&
      testAbility(`active`, ability.hexerei.id) &&
      moveType == "ghost"
    )
      totalPower *= 1.3;
    if (
      below50hp &&
      testAbility(`active`, ability.glimmer.id) &&
      moveType == "fairy"
    )
      totalPower *= 1.3;
    if (
      below50hp &&
      testAbility(`active`, ability.skyward.id) &&
      moveType == "flying"
    )
      totalPower *= 1.3;
    if (
      below50hp &&
      testAbility(`active`, ability.draconic.id) &&
      moveType == "dragon"
    )
      totalPower *= 1.3;
    if (
      below50hp &&
      testAbility(`active`, ability.noxious.id) &&
      moveType == "poison"
    )
      totalPower *= 1.3;
    if (
      below50hp &&
      testAbility(`active`, ability.solid.id) &&
      moveType == "rock"
    )
      totalPower *= 1.3;
    if (
      below50hp &&
      testAbility(`active`, ability.rime.id) &&
      moveType == "ice"
    )
      totalPower *= 1.3;
    if (
      below50hp &&
      testAbility(`active`, ability.voltage.id) &&
      moveType == "electric"
    )
      totalPower *= 1.3;

    if (
      testAbility(`active`, ability.rivalry.id) &&
      pkmn[saved.currentPkmn].type.some((t) =>
        pkmn[team[exploreActiveMember].pkmn.id].type.includes(t),
      )
    )
      totalPower *= 1.5;

    if (testAbility(`active`, ability.sheerForce.id) && nextMove.hitEffect)
      totalPower *= 1.2;

    if (
      testAbility(`active`, ability.hugePower.id) &&
      nextMove.split == "physical"
    )
      totalPower *= 2;

    if (
      testAbility(`active`, ability.toxicBoost.id) &&
      team[exploreActiveMember].buffs?.poisoned > 0
    )
      totalPower *= 1.2;
    if (
      testAbility(`active`, ability.flareBoost.id) &&
      team[exploreActiveMember].buffs?.burn > 0
    )
      totalPower *= 1.2;

    if (
      team[exploreActiveMember].item == undefined &&
      testAbility(`active`, ability.unburden.id)
    )
      team[exploreActiveMember].buffs.speup1 = 10;
    ...
    if (
      (testAbility(`active`, ability.quarkDrive.id) &&
        saved.weatherTimer > 0 &&
        saved.weather == "electricTerrain") ||
      (testAbility(`active`, ability.protosynthesis.id) &&
        saved.weatherTimer > 0 &&
        saved.weather == "sunny")
    ) {
      if (returnHighestStat(team[exploreActiveMember].pkmn) == "spe")
    ...
    if (
      saved.weatherTimer > 0 &&
      saved.weather == "sandstorm" &&
      (moveType == "rock" || moveType == "ground")
    )
      totalPower *= 1.75;
    if (saved.weatherTimer > 0 && saved.weather == "hail" && moveType == "ice")
      totalPower *= 1.75;
    if (
      saved.weatherTimer > 0 &&
      saved.weather == "foggy" &&
      (moveType == "ghost" || moveType == "dark")
    )
      totalPower *= 1.75;
    if (
      saved.weatherTimer > 0 &&
      saved.weather == "electricTerrain" &&
      (moveType == "electric" || moveType == "steel")
    )
      totalPower *= 1.75;
    if (
      saved.weatherTimer > 0 &&
      saved.weather == "mistyTerrain" &&
      (moveType == "fairy" || moveType == "psychic")
    )
      totalPower *= 1.75;
    if (
      saved.weatherTimer > 0 &&
      saved.weather == "grassyTerrain" &&
      (moveType == "grass" || moveType == "bug")
    )
      totalPower *= 1.75;

    if (saved.weatherTimer > 0) {
      if (
        testAbility("active", ability.chlorophyll.id) &&
        saved.weather == "sunny"
      ) {
        moveBuff("wild", "speup1", "self");
      }
      if (
        testAbility("active", ability.slushRush.id) &&
        saved.weather == "hail"
      ) {
        moveBuff("wild", "speup1", "self");
      }
      if (
        testAbility("active", ability.swiftSwim.id) &&
        saved.weather == "rainy"
      ) {
        moveBuff("wild", "speup1", "self");
      }
      if (
        testAbility("active", ability.sandRush.id) &&
        saved.weather == "sandstorm"
      ) {
        moveBuff("wild", "speup1", "self");
      }
      if (
        testAbility("active", ability.moltShed.id) &&
        saved.weather == "grassyTerrain"
      ) {
        moveBuff("wild", "speup1", "self");
      }
      if (
        testAbility("active", ability.faeRush.id) &&
        saved.weather == "mistyTerrain"
      ) {
        moveBuff("wild", "speup1", "self");
      }
      if (
        testAbility("active", ability.hyperconductor.id) &&
        saved.weather == "electricTerrain"
      ) {
        moveBuff("wild", "speup1", "self");
      }
      if (
        testAbility("active", ability.intangible.id) &&
        saved.weather == "foggy"
      ) {
        moveBuff("wild", "speup1", "self");
      }
      ...
      if (
        testAbility("active", ability.solarPower.id) &&
        saved.weather == "sunny"
      ) {
        moveBuff("wild", "satkup1", "self");
      }
      if (
        testAbility("active", ability.iceBody.id) &&
        saved.weather == "hail"
      ) {
        moveBuff("wild", "defup1", "self");
      }
      if (
        testAbility("active", ability.rainDish.id) &&
        saved.weather == "rainy"
      ) {
        moveBuff("wild", "satkup1", "self");
      }
      if (
        testAbility("active", ability.sandForce.id) &&
        saved.weather == "sandstorm"
      ) {
        moveBuff("wild", "atkup1", "self");
      }
    ...
    if (hasAnyStatus) {
      if (testAbility("active", ability.marvelScale.id))
        moveBuff("wild", "defup1", "self");
      if (testAbility("active", ability.livingShield.id))
        moveBuff("wild", "sdefup1", "self");
      if (testAbility("active", ability.guts.id))
        moveBuff("wild", "atkup1", "self");
      if (testAbility("active", ability.brittleArmor.id))
        moveBuff("wild", "satkup1", "self");
    }
    ...
    if (
      team[exploreActiveMember].damageDealt == undefined ||
      team[exploreActiveMember].damageDealt == NaN
    )
      team[exploreActiveMember].damageDealt = 0;
    ...
    if (testAbility(`active`, ability.magicGuard.id) == false) {
      ...
      if (areas[saved.currentArea].id != areas.training.id) {
        if (
          ...
          attacker.playerHp -= attacker.playerHpMax / dotDamage;
        }
        if (
          team[exploreActiveMember].buffs?.poisoned > 0 &&
          !testAbility(`active`, ability.toxicBoost.id)
    ...
    if (team[exploreActiveMember].item == item.lifeOrb.id) {
      attacker.playerHp -= attacker.playerHpMax / 10;
    }

    for (const i in team[exploreActiveMember].buffs) {
      if (team[exploreActiveMember].buffs[i] > 0)
        team[exploreActiveMember].buffs[i] -= 1;
    }

    if (testAbility(`active`, ability.moody.id)) {
      let picked = arrayPick(
        ["atkup2", "satkup2", "defup2", "sdefup2", "speup2"],
        2,
      );
    ...
    for (let i = 0; i < multihit; i++) {
      if (
        !(
          team[exploreActiveMember].buffs?.freeze > 0 ||
          team[exploreActiveMember].buffs?.sleep > 0
        )
      ) {
        if (
          testAbility(`active`, ability.sheerForce.id) == false ||
          (testAbility(`active`, ability.sheerForce.id) &&
            (totalPower == 0 ||
              nextMove.unaffectedBy?.includes(ability.sheerForce.id)))
        ) {
          if (
            nextMove.hitEffect &&
            (typeEffectiveness(moveType, pkmn[saved.currentPkmn].type) != 0 ||
              totalPower == 0 ||
              testAbility(`active`, ability.noGuard.id))
          ) {
    ...
    if (
      testAbility(
        `active`,
        ability.sereneGrace.id &&
          !nextMove.unaffectedBy?.includes(ability.sereneGrace.id),
      ) &&
      nextMove.hitEffect
    )
      nextMove.hitEffect("wild");
    if (
      testAbility(
        `active`,
        ability.parentalBond.id &&
          !nextMove.unaffectedBy?.includes(ability.parentalBond.id),
      ) &&
      nextMove.hitEffect
    )
      nextMove.hitEffect("wild");
    ...
    const fractionDamage =
      100 +
      attacker.bst.hp * 30 * Math.pow(1.15, attacker.ivs.hp) +
      attacker.bst.def * 15 * Math.pow(1.15, attacker.ivs.def) +
      attacker.bst.sdef * 15 * Math.pow(1.15, attacker.ivs.sdef);
...
function typeEffectiveness(attacking, defending) {
  ...
  const result = defending.reduce(
    (mul, defType) => mul * (chart[attacking]?.[defType] ?? 1),
    1,
  );
  if (
    (saved.currentArea == areas.frontierSpiralingTower.id ||
...
function typeWeak(myType1, myType2 = null, ranking = 1) {
  //ts was made by gpt ask him about it
  const allTypes = [
    "normal",
    "fire",
    "water",
...
function exploreCombatWild() {
  ...
  if (
    nextMoveBoxWild !=
    document?.getElementById(`pkmn-movebox-wild-${exploreCombatWildTurn}`)
  )
    nextMoveBoxWild = document?.getElementById(
      `pkmn-movebox-wild-${exploreCombatWildTurn}`,
    );
  if (nextMoveWild != nextMoveBoxWild?.dataset?.move)
    nextMoveWild = nextMoveBoxWild?.dataset?.move;
  if (moveTimerWild != move[nextMoveWild]?.timer)
    moveTimerWild = move[nextMoveWild]?.timer;
  if (
    barWild !=
    document.getElementById(`pkmn-movebox-wild-${exploreCombatWildTurn}-bar`)
  )
    barWild = document.getElementById(
      `pkmn-movebox-wild-${exploreCombatWildTurn}-bar`,
    );
  ...
  if (
    saved.overrideBattleTimer != defaultPlayerMoveTimer &&
    moveTimerWild != saved.overrideBattleTimer
  )
    moveTimerWild = saved.overrideBattleTimer;
  ...
  if (areas[saved.currentArea].id == areas.training.id)
    speedStars = returnDivisionStars(pkmn[saved.currentPkmn]);

  barProgressWild +=
    100 / ((moveTimerWild * Math.pow(0.9, speedStars)) / (1000 / 60));
  ...
  if (barProgressWild < 99) {
    ...
    if (move[nextMoveWild].split == "physical") {
      totalPower =
        (move[nextMoveWild].power +
          Math.max(
            0,
            pkmn[saved.currentPkmn].bst.atk * 30 -
              pkmn[team[exploreActiveMember].pkmn.id].bst.def *
                30 *
                Math.pow(1.1, pkmn[team[exploreActiveMember].pkmn.id].ivs.def),
          )) *
        (1 + wildLevel * 0.1) *
        1;
      ...
      if (areas[saved.currentArea].id == areas.training.id) {
        totalPower =
          (move[nextMoveWild].power +
            Math.max(
              0,
              returnDivisionStars(pkmn[saved.currentPkmn]) * 30 -
                returnDivisionStars(pkmn[saved.currentPkmn]) *
                  30 *
                  Math.pow(
                    1.1,
                    pkmn[team[exploreActiveMember].pkmn.id].ivs.def,
                  ),
            )) *
          (1 + wildLevel * 0.1) *
          1;
    ...
    if (move[nextMoveWild].split == "special") {
      totalPower =
        (move[nextMoveWild].power +
          Math.max(
            0,
            pkmn[saved.currentPkmn].bst.satk * 30 -
              pkmn[team[exploreActiveMember].pkmn.id].bst.sdef *
                30 *
                Math.pow(1.1, pkmn[team[exploreActiveMember].pkmn.id].ivs.sdef),
          )) *
        (1 + wildLevel * 0.1) *
        1;
            ...
            Math.max(
              ...
              returnDivisionStars(pkmn[saved.currentPkmn]) * 30 -
                returnDivisionStars(pkmn[saved.currentPkmn]) *
                  ...
                  Math.pow(
                    1.1,
                    pkmn[team[exploreActiveMember].pkmn.id].ivs.sdef,
                  ),
            )) *
          (1 + wildLevel * 0.1) *
          1;
    ...
    if (
      typeEffectiveness(
        move[nextMoveWild].type,
        pkmn[team[exploreActiveMember].pkmn.id].type,
      ) > 1
    )
      superEffective = true;

    //stab
    if (pkmn[saved.currentPkmn].type.includes(move[nextMoveWild].type))
      totalPower *= 1.5;
    ...
    if (typeMultiplier == 1.5 && testAbility(`active`, ability.filter.id))
      typeMultiplier = 1.25;
    if (typeMultiplier == 2.25 && testAbility(`active`, ability.filter.id))
      typeMultiplier = 1.65;

    if (typeMultiplier > 1 && testAbility(`active`, ability.angerPoint.id))
      moveBuff("wild", "atkup2", "self");
    if (typeMultiplier > 1 && testAbility(`active`, ability.justified.id))
      moveBuff("wild", "satkup2", "self");

    totalPower *= typeMultiplier;

    if (
      team[exploreActiveMember].item == item.occaBerry.id &&
      move[nextMoveWild].type == "fire" &&
      superEffective
    ) {
      totalPower /= item.occaBerry.power() / 100 + 1;
    }
    if (
      team[exploreActiveMember].item == item.passhoBerry.id &&
      move[nextMoveWild].type == "water" &&
      superEffective
    ) {
      totalPower /= item.passhoBerry.power() / 100 + 1;
    }
    if (
      team[exploreActiveMember].item == item.wacanBerry.id &&
      move[nextMoveWild].type == "electric" &&
      superEffective
    ) {
      totalPower /= item.wacanBerry.power() / 100 + 1;
    }
    if (
      team[exploreActiveMember].item == item.rindoBerry.id &&
      move[nextMoveWild].type == "grass" &&
      superEffective
    ) {
      totalPower /= item.rindoBerry.power() / 100 + 1;
    }
    if (
      team[exploreActiveMember].item == item.yacheBerry.id &&
      move[nextMoveWild].type == "ice" &&
      superEffective
    ) {
      totalPower /= item.yacheBerry.power() / 100 + 1;
    }
    if (
      team[exploreActiveMember].item == item.chopleBerry.id &&
      move[nextMoveWild].type == "fighting" &&
      superEffective
    ) {
      totalPower /= item.chopleBerry.power() / 100 + 1;
    }
    if (
      team[exploreActiveMember].item == item.kebiaBerry.id &&
      move[nextMoveWild].type == "poison" &&
      superEffective
    ) {
      totalPower /= item.kebiaBerry.power() / 100 + 1;
    }
    if (
      team[exploreActiveMember].item == item.shucaBerry.id &&
      move[nextMoveWild].type == "ground" &&
      superEffective
    ) {
      totalPower /= item.shucaBerry.power() / 100 + 1;
    }
    if (
      team[exploreActiveMember].item == item.cobaBerry.id &&
      move[nextMoveWild].type == "flying" &&
      superEffective
    ) {
      totalPower /= item.cobaBerry.power() / 100 + 1;
    }
    if (
      team[exploreActiveMember].item == item.payapaBerry.id &&
      move[nextMoveWild].type == "psychic" &&
      superEffective
    ) {
      totalPower /= item.payapaBerry.power() / 100 + 1;
    }
    if (
      team[exploreActiveMember].item == item.tangaBerry.id &&
      move[nextMoveWild].type == "bug" &&
      superEffective
    ) {
      totalPower /= item.tangaBerry.power() / 100 + 1;
    }
    if (
      team[exploreActiveMember].item == item.chartiBerry.id &&
      move[nextMoveWild].type == "rock" &&
      superEffective
    ) {
      totalPower /= item.chartiBerry.power() / 100 + 1;
    }
    if (
      team[exploreActiveMember].item == item.kasibBerry.id &&
      move[nextMoveWild].type == "ghost" &&
      superEffective
    ) {
      totalPower /= item.kasibBerry.power() / 100 + 1;
    }
    if (
      team[exploreActiveMember].item == item.habanBerry.id &&
      move[nextMoveWild].type == "dragon" &&
      superEffective
    ) {
      totalPower /= item.habanBerry.power() / 100 + 1;
    }
    if (
      team[exploreActiveMember].item == item.colburBerry.id &&
      move[nextMoveWild].type == "dark" &&
      superEffective
    ) {
      totalPower /= item.colburBerry.power() / 100 + 1;
    }
    if (
      team[exploreActiveMember].item == item.babiriBerry.id &&
      move[nextMoveWild].type == "steel" &&
      superEffective
    ) {
      totalPower /= item.babiriBerry.power() / 100 + 1;
    }
    if (
      team[exploreActiveMember].item == item.roseliBerry.id &&
      move[nextMoveWild].type == "fairy" &&
      superEffective
    ) {
      totalPower /= item.roseliBerry.power() / 100 + 1;
    }

    if (
      team[exploreActiveMember].item == item.eviolite.id &&
      pkmn[team[exploreActiveMember].pkmn.id].evolve !== undefined &&
      (pkmn[team[exploreActiveMember].pkmn.id]
        .evolve()[1]
        .pkmn.id.slice(0, 4) !== "mega" ||
        team[exploreActiveMember].pkmn.id == pkmn.bayleef.id)
    ) {
      totalPower /= item.eviolite.power();
    }

    if (team[exploreActiveMember].item == item.mentalHerb.id) {
      totalPower /= item.mentalHerb.power() / 100 + 1;
    }
    ...
    //abilities
    if (
      testAbility(`active`, ability.grabGuard.id) &&
      move[nextMoveWild].type == "fighting"
    )
      totalPower /= 2;
    if (
      testAbility(`active`, ability.waterGuard.id) &&
      move[nextMoveWild].type == "water"
    )
      totalPower /= 2;
    if (
      testAbility(`active`, ability.flameGuard.id) &&
      move[nextMoveWild].type == "fire"
    )
      totalPower /= 2;
    if (
      testAbility(`active`, ability.curseGuard.id) &&
      move[nextMoveWild].type == "ghost"
    )
      totalPower /= 2;
    if (
      testAbility(`active`, ability.poisonGuard.id) &&
      move[nextMoveWild].type == "poison"
    )
      totalPower /= 2;
    if (
      testAbility(`active`, ability.iceGuard.id) &&
      move[nextMoveWild].type == "ice"
    )
      totalPower /= 2;
    if (
      testAbility(`active`, ability.psychicGuard.id) &&
      move[nextMoveWild].type == "psychic"
    )
      totalPower /= 2;
    if (
      testAbility(`active`, ability.fairyGuard.id) &&
      move[nextMoveWild].type == "fairy"
    )
      totalPower /= 2;
    if (
      testAbility(`active`, ability.leafGuard.id) &&
      move[nextMoveWild].type == "grass"
    )
      totalPower /= 2;
    if (
      testAbility(`active`, ability.plainGuard.id) &&
      move[nextMoveWild].type == "normal"
    )
      totalPower /= 2;
    if (
      testAbility(`active`, ability.sinisterGuard.id) &&
      move[nextMoveWild].type == "dark"
    )
      totalPower /= 2;
    if (
      testAbility(`active`, ability.steelGuard.id) &&
      move[nextMoveWild].type == "steel"
    )
      totalPower /= 2;
    if (
      testAbility(`active`, ability.dragonGuard.id) &&
      move[nextMoveWild].type == "dragon"
    )
      totalPower /= 2;
    if (
      testAbility(`active`, ability.rockGuard.id) &&
      move[nextMoveWild].type == "rock"
    )
      totalPower /= 2;
    if (
      testAbility(`active`, ability.groundGuard.id) &&
      move[nextMoveWild].type == "ground"
    )
      totalPower /= 2;
    if (
      testAbility(`active`, ability.flyingGuard.id) &&
      move[nextMoveWild].type == "flying"
    )
      totalPower /= 2;

    if (
      testAbility(`active`, ability.voltAbsorb.id) &&
      move[nextMoveWild].type == "electric"
    )
      totalPower = 0;
    if (
      testAbility(`active`, ability.waterAbsorb.id) &&
      move[nextMoveWild].type == "water"
    )
      totalPower = 0;
    if (
      testAbility(`active`, ability.flareAbsorb.id) &&
      move[nextMoveWild].type == "fire"
    )
      totalPower = 0;
    if (
      testAbility(`active`, ability.curseAbsorb.id) &&
      move[nextMoveWild].type == "ghost"
    )
      totalPower = 0;
    if (
      testAbility(`active`, ability.poisonAbsorb.id) &&
      move[nextMoveWild].type == "poison"
    )
      totalPower = 0;
    if (
      testAbility(`active`, ability.frostAbsorb.id) &&
      move[nextMoveWild].type == "ice"
    )
      totalPower = 0;
    if (
      testAbility(`active`, ability.psychicAbsorb.id) &&
      move[nextMoveWild].type == "psychic"
    )
      totalPower = 0;
    if (
      testAbility(`active`, ability.lightAbsorb.id) &&
      move[nextMoveWild].type == "fairy"
    )
      totalPower = 0;
    if (
      testAbility(`active`, ability.growthAbsorb.id) &&
      move[nextMoveWild].type == "grass"
    )
      totalPower = 0;

    if (
      testAbility(`active`, ability.flashElectro.id) &&
      move[nextMoveWild].type == "electric"
    ) {
      totalPower = 0;
      moveBuff("wild", "speup1", "self");
    ...
    if (
      testAbility(`active`, ability.flashAqua.id) &&
      move[nextMoveWild].type == "water"
    ) {
      totalPower = 0;
      moveBuff("wild", "speup1", "self");
    ...
    if (
      testAbility(`active`, ability.flashPyro.id) &&
      move[nextMoveWild].type == "fire"
    ) {
      totalPower = 0;
      moveBuff("wild", "speup1", "self");
    ...
    if (
      testAbility(`active`, ability.flashUmbra.id) &&
      move[nextMoveWild].type == "ghost"
    ) {
      totalPower = 0;
      moveBuff("wild", "speup1", "self");
    ...
    if (
      testAbility(`active`, ability.flashVenum.id) &&
      move[nextMoveWild].type == "poison"
    ) {
      totalPower = 0;
      moveBuff("wild", "speup1", "self");
    ...
    if (
      testAbility(`active`, ability.flashCryo.id) &&
      move[nextMoveWild].type == "ice"
    ) {
      totalPower = 0;
      moveBuff("wild", "speup1", "self");
    ...
    if (
      testAbility(`active`, ability.flashPsycha.id) &&
      move[nextMoveWild].type == "psychic"
    ) {
      totalPower = 0;
      moveBuff("wild", "speup1", "self");
    ...
    if (
      testAbility(`active`, ability.flashFae.id) &&
      move[nextMoveWild].type == "fairy"
    ) {
      totalPower = 0;
      moveBuff("wild", "speup1", "self");
    ...
    if (
      testAbility(`active`, ability.flashHerba.id) &&
      move[nextMoveWild].type == "grass"
    ) {
      totalPower = 0;
      moveBuff("wild", "speup1", "self");
    ...
    if (testAbility(`active`, ability.static.id) && rng(0.1))
      moveBuff("wild", "paralysis");
    if (testAbility(`active`, ability.flameBody.id) && rng(0.1))
      moveBuff("wild", "burn");
    if (testAbility(`active`, ability.poisonPoint.id) && rng(0.1))
      moveBuff("wild", "poisoned");
    if (testAbility(`active`, ability.strangeCharm.id) && rng(0.1))
      moveBuff("wild", "confused");
    if (testAbility(`active`, ability.effectSpore.id) && rng(0.05))
      moveBuff("wild", "sleep");
    if (testAbility(`active`, ability.glacialBody.id) && rng(0.05))
      moveBuff("wild", "freeze");

    const below50hp =
      pkmn[team[exploreActiveMember].pkmn.id]?.playerHp <
      pkmn[team[exploreActiveMember].pkmn.id]?.playerHpMax * 0.5;
    if (!below50hp && testAbility(`active`, ability.multiscale.id))
      totalPower /= 2;

    if (
      testAbility(`active`, ability.levitate.id) &&
      move[nextMoveWild].type === "ground"
    )
      totalPower = 0;
    if (
      testAbility(`active`, ability.thickFat.id) &&
      (move[nextMoveWild].type === "fire" || move[nextMoveWild].type === "ice")
    )
      totalPower /= 2;
    ...
    if (
      saved.weatherTimer > 0 &&
      saved.weather == "sandstorm" &&
      (move[nextMoveWild].type == "rock" || move[nextMoveWild].type == "ground")
    )
      totalPower *= 1.75;
    if (
      saved.weatherTimer > 0 &&
    ...
    if (
      saved.weatherTimer > 0 &&
      saved.weather == "foggy" &&
      (move[nextMoveWild].type == "ghost" || move[nextMoveWild].type == "dark")
    )
      totalPower *= 1.75;
    if (
      saved.weatherTimer > 0 &&
    ...
    if (
      saved.weatherTimer > 0 &&
      saved.weather == "grassyTerrain" &&
      (move[nextMoveWild].type == "grass" || move[nextMoveWild].type == "bug")
    )
      totalPower *= 1.75;

    if (testAbility(`active`, ability.wonderGuard.id) && !superEffective)
      totalPower *= 0.2;
    ...
    if (wildBuffs.burn > 0 && testAbility("active", ability.scorch.id))
      dotDamage /= 2;
    if (wildBuffs.poisoned > 0 && testAbility("active", ability.corrosion.id))
      dotDamage /= 2;

    if (wildBuffs.burn > 0)
      team[exploreActiveMember].damageDealt += Math.min(
        wildPkmnHpMax / dotDamage,
      );
    if (wildBuffs.poisoned > 0)
      team[exploreActiveMember].damageDealt += Math.min(
        wildPkmnHpMax / dotDamage,
      );

    if (wildBuffs.burn > 0) {
      wildPkmnHp -= wildPkmnHpMax / dotDamage;
      updateWildPkmn();
    }
    if (wildBuffs.poisoned > 0) {
      wildPkmnHp -= wildPkmnHpMax / dotDamage;
...
function initialiseArea() {
  if (saved.currentArea == areas.training.id && areas.training.tier == 1)
    currentTrainingWave = 30;
  if (saved.currentArea == areas.training.id && areas.training.tier == 2)
    currentTrainingWave = 30;
  if (saved.currentArea == areas.training.id && areas.training.tier == 3)
    currentTrainingWave = 30;
  ...
  for (const moveID in move)
    if (move[moveID].buildup !== undefined) move[moveID].buildup = 0;
  ...
  if (testAbility(`active`, ability.electricSurge.id))
    changeWeather("electricTerrain");
  if (testAbility(`active`, ability.grassySurge.id))
    changeWeather("grassyTerrain");
  if (testAbility(`active`, ability.mistySurge.id))
    changeWeather("mistyTerrain");
}
...
function setWildAreas() {
  ...
  document.getElementById("explore-menu-header").style.backgroundImage =
    "url(img/bg/forest.png)";
  ...
  if (
    ...
    const uncommonPick = []
      .concat(arrayPick(wildlifePoolUncommon, 1))
      .map((name) => pkmn[name]);

    areas.wildlifePark.spawns.common = arrayPick(wildlifePoolCommon, 3).map(
      (name) => pkmn[name],
    );
    areas.wildlifePark.spawns.uncommon = uncommonPick;
    areas.wildlifePark.spawns.rare = []
      .concat(arrayPick(wildlifePoolRare, 1))
      .map((name) => pkmn[name]);
  ...
  for (const i in areas.wildlifePark.spawns) {
    for (const e in areas.wildlifePark.spawns[i]) {
      if (pkmn[areas.wildlifePark.spawns[i][e].id].caught <= 0)
        completionMark = false;
      if (pkmn[areas.wildlifePark.spawns[i][e].id].shiny != true)
        shinyMark = false;
    }
  }
  ...
  for (const i in areas) {
    ...
    if (
      areas[i].unlockRequirement == undefined ||
      areas[i].unlockRequirement()
    ) {
    ...
    if (areas[i].unlockRequirement && !areas[i].unlockRequirement())
      unlockRequirement = `<span class="ticket-unlock">
...
function setEventAreas() {
  ...
  if (rotationEventCurrent == 1)
    document.getElementById("event-banner").style.backgroundImage =
      "url(img/bg/event/kanto.png)";
  if (rotationEventCurrent == 2)
    document.getElementById("event-banner").style.backgroundImage =
      "url(img/bg/event/past.png)";
  if (rotationEventCurrent == 3)
    document.getElementById("event-banner").style.backgroundImage =
      "url(img/bg/event/ancient.png)";
  if (rotationEventCurrent == 4)
    document.getElementById("event-banner").style.backgroundImage =
      "url(img/bg/event/aether.jpg)";
  if (rotationEventCurrent == 5)
    document.getElementById("event-banner").style.backgroundImage =
      "url(img/bg/event/future.jpg)";
  ...
  document.getElementById("event-selector-" + eventCategory).style =
    `background: #91718B; outline: solid 1px #F97DFF; color: white`;
  ...
  for (const i in areas) {
    ...
    if (
      !Array.isArray(areas[i].rotation) &&
      areas[i].rotation !== rotationEventCurrent
    )
      continue;
    if (
      Array.isArray(areas[i].rotation) &&
      !areas[i].rotation.includes(rotationEventCurrent)
    )
      continue;
    ...
    if (
      ...
      divAreas.addEventListener("click", (e) => {
        if (
          areas[i].encounter &&
          areas[i].difficulty === tier2difficulty &&
          areas.vsEliteFourLance.defeated != true
        )
          return;
        saved.currentAreaBuffer = i;
        document.getElementById(`preview-team-exit`).style.display = "flex";
...
let lastEventPeriod = Math.floor(Math.floor(Date.now() / 86400000) / 3);
...
document.getElementById("explore-pkmn-tag").addEventListener("change", (e) => {
  pkmn[currentEditedPkmn].tag =
    document.getElementById("explore-pkmn-tag").value;
  if (document.getElementById("explore-pkmn-tag").value == "none")
    pkmn[currentEditedPkmn].tag = undefined;
  updatePokedex();
});

document
  .getElementById("pokedex-filter-tag")
  .addEventListener("change", (e) => {
    updatePokedex();
  });

document
  .getElementById("pokedex-filter-type")
  .addEventListener("change", (e) => {
    updatePokedex();
  });

document
  .getElementById("pokedex-filter-type-2")
  .addEventListener("change", (e) => {
    updatePokedex();
  });

document
  .getElementById("pokedex-filter-level")
  .addEventListener("change", (e) => {
    updatePokedex();
  });

document
  .getElementById("pokedex-level-sort")
  .addEventListener("change", (e) => {
    // Force sort mode to level when toggling level order; reset when cleared
    if (e.target.value === "default")
      document.getElementById("pokedex-sort-filter").value = "default";
    else document.getElementById("pokedex-sort-filter").value = "level";
    updatePokedex();
  });

document
  .getElementById("pokedex-filter-ability")
  .addEventListener("change", (e) => {
    updatePokedex();
  });

document
  .getElementById("pokedex-filter-shiny")
  .addEventListener("change", (e) => {
    updatePokedex();
  });

document
  .getElementById("pokedex-filter-signature")
  .addEventListener("change", (e) => {
    updatePokedex();
  });

document
  .getElementById("pokedex-filter-ribbon")
  .addEventListener("change", (e) => {
    updatePokedex();
  });

document
  .getElementById("pokedex-filter-division")
  .addEventListener("change", (e) => {
    updatePokedex();
  });

document
  .getElementById("pokedex-filter-evolution")
  .addEventListener("change", (e) => {
    updatePokedex();
  });
...
function resetPokedexFilters() {
  ...
  document.getElementById("pokedex-level-sort").value = "default";
...
document
  .getElementById("pokedex-sort-filter")
  .addEventListener("change", (e) => {
...
function updatePokedex() {
  ...
  if (dexTeamSelect != undefined) {
    ...
    document.getElementById("pokedex-filters-title").innerHTML =
  ...
  if (tmToTeach != undefined) {
    ...
    document.getElementById("pokedex-filters-cancel").style.display = "flex";
    document.getElementById("pokedex-filters-title").innerHTML =
      `Select a Pokemon to teach ${format(tmToTeach)}`;
  }
  ...
  if (evoItemToUse != undefined) {
    ...
    document.getElementById("pokedex-filters-cancel").style.display = "flex";
    document.getElementById("pokedex-filters-title").innerHTML =
      `Select a Pokemon to use the ${format(evoItemToUse)}`;
  }
  ...
  if (vitaminToUse != undefined) {
    ...
    document.getElementById("pokedex-filters-cancel").style.display = "flex";
    document.getElementById("pokedex-filters-title").innerHTML =
      `Select a Pokemon to use the ${format(vitaminToUse)}`;
  }
  ...
  if (itemToUse != undefined) {
    ...
    document.getElementById("pokedex-filters-cancel").style.display = "flex";
    document.getElementById("pokedex-filters-title").innerHTML =
      `Select a Pokemon to use the ${format(itemToUse)}`;
  }
  ...
  for (const i in pkmn) {
    //filters
    if (pkmn[i].ability == undefined)
      pkmn[i].ability = learnPkmnAbility(pkmn[i].id);
    if (
      document.getElementById(`pokedex-filter-type`).value !== "all" &&
      !pkmn[i].type.includes(
        document.getElementById(`pokedex-filter-type`).value,
      )
    )
      continue;
    if (
      document.getElementById(`pokedex-filter-type-2`).value !== "all" &&
      !pkmn[i].type.includes(
        document.getElementById(`pokedex-filter-type-2`).value,
      )
    )
      continue;
    if (
      ...
      !(
        pkmn[i].level <=
          document.getElementById(`pokedex-filter-level`).value &&
        pkmn[i].level >=
          document.getElementById(`pokedex-filter-level`).value - 19
      )
    )
      continue;
    if (
      document.getElementById(`pokedex-filter-ability`).value !== "all" &&
      document.getElementById(`pokedex-filter-ability`).value != 4 &&
      ability[pkmn[i].ability].rarity !=
        document.getElementById(`pokedex-filter-ability`).value
    )
      continue;
    if (
      document.getElementById(`pokedex-filter-ability`).value == "4" &&
      (pkmn[i].hiddenAbilityUnlocked == true ||
        pkmn[i].hiddenAbility == undefined)
    )
      continue;
    if (
      (v = document.getElementById("pokedex-filter-shiny").value) !== "all" &&
      pkmn[i].shiny != (v === "true" ? true : undefined)
    )
      continue;
    if (
      document.getElementById(`pokedex-filter-division`).value !== "all" &&
      returnPkmnDivision(pkmn[i]) !=
        document.getElementById(`pokedex-filter-division`).value
    )
      continue;
    if (
      document.getElementById(`pokedex-filter-tag`).value !== "all" &&
      document.getElementById(`pokedex-filter-tag`).value !== "none" &&
      pkmn[i].tag !== document.getElementById(`pokedex-filter-tag`).value
    )
      continue;
    if (
      document.getElementById(`pokedex-filter-tag`).value == "none" &&
      pkmn[i].tag != undefined
    )
      continue;
    if (
      document.getElementById(`pokedex-filter-ribbon`).value !== "all" &&
      pkmn[i].ribbons == undefined
    )
      continue;
    if (
      document.getElementById(`pokedex-filter-tag`).value !== "hidden" &&
      pkmn[i].tag == "hidden"
    )
      continue;

    if (
      document.getElementById(`pokedex-filter-signature`).value == "false" &&
      pkmn[i].signature == undefined
    )
      continue;
    ...
    if (pkmn[i].evolve !== undefined) {
      for (const evo in pkmn[i]?.evolve()) {
        ...
        if (pkmn[i].evolve()[evo].pkmn.caught == 0) {
          ...
          if (pkmn[i].evolve()[evo].level !== undefined)
            missingLevelEvolution = true;
        }
      }
    }
    ...
    if (
      document.getElementById(`pokedex-filter-evolution`).value ==
        "level-only" &&
      !missingLevelEvolution
    )
      continue;
    ...
    if (
      areas[saved.currentAreaBuffer]?.type == "frontier" &&
      rotationFrontierCurrent === 1 &&
      returnPkmnDivision(pkmn[i]) != "C" &&
      returnPkmnDivision(pkmn[i]) != "D"
    )
      continue;
    if (
      areas[saved.currentAreaBuffer]?.type == "frontier" &&
      rotationFrontierCurrent === 2 &&
      returnPkmnDivision(pkmn[i]) != "B" &&
      returnPkmnDivision(pkmn[i]) != "C" &&
      returnPkmnDivision(pkmn[i]) != "D"
    )
      continue;
    if (
      areas[saved.currentAreaBuffer]?.type == "frontier" &&
      rotationFrontierCurrent === 3 &&
      returnPkmnDivision(pkmn[i]) != "A" &&
      returnPkmnDivision(pkmn[i]) != "B" &&
      returnPkmnDivision(pkmn[i]) != "C" &&
      returnPkmnDivision(pkmn[i]) != "D"
    )
  ...
  const levelSortOrder = document.getElementById("pokedex-level-sort")?.value || "desc";
  const sortComparator = (a, b) => {
    if (sort === "level")
      return levelSortOrder === "asc" ? a.level - b.level : b.level - a.level;
  ...
  if (sort !== "default") {
    sortedPokemon.sort(sortComparator);
  }
  ...
  for (const p of sortedPokemon) {
    ...
    if (pkmn[i].shiny)
      nameMarks += `<strong style="color:${markColor}; margin-left:0.2rem">✦</strong>`;
    else {
      if (pkmn[i].tag == "red")
        nameMarks += `<strong style="color:${markColor}; margin-left:0.2rem">⬤</strong>`;
      if (pkmn[i].tag == "orange")
        nameMarks += `<strong style="color:${markColor}; margin-left:0.2rem">⬤</strong>`;
      if (pkmn[i].tag == "yellow")
        nameMarks += `<strong style="color:${markColor}; margin-left:0.2rem">⬤</strong>`;
      if (pkmn[i].tag == "green")
        nameMarks += `<strong style="color:${markColor}; margin-left:0.2rem">⬤</strong>`;
      if (pkmn[i].tag == "lime")
    ...
    if (pkmn[i].shiny)
      div.innerHTML = `<span style="display:flex; white-space:nowrap">lvl ${pkmn[i].level} ${nameMarks}</span> <img class="sprite-trim" src="img/pkmn/shiny/${i}.png">`;
    if (pkmn[i].shiny && pkmn[i].shinyDisabled == true)
      div.innerHTML = `<span style="display:flex; white-space:nowrap">lvl ${pkmn[i].level} ${nameMarks}</span><img class="sprite-trim" src="img/pkmn/sprite/${i}.png">`;

    if (dexTeamSelect !== undefined) {
      //preview team display
      document.getElementById(`pokedex-filters-cancel`).style.display = "flex";
    }
    ...
    if (tmToTeach != undefined) {
      ...
      if (
        !move[tmToTeach].moveset.includes("all") &&
        !move[tmToTeach].moveset.some((t) => pkmn[i].type.includes(t))
      )
        continue;
    ...
    if (evoItemToUse != undefined) {
      ...
      for (const evo in pkmn[i].evolve()) {
        if (
          pkmn[i].evolve()[evo].item !== undefined &&
          pkmn[i].evolve()[evo].item.id === evoItemToUse
        )
          hidePkmn = false;
        ...
        if (pkmn[i].evolve()[evo].pkmn.id.slice(0, 4) === "mega")
          levelToEvolve = 100;
        ...
        div.addEventListener("click", (e) => {
          ...
          document.getElementById("tooltipBottom").innerHTML =
            `${format(pkmn[pkmn[i].evolve()[evo].pkmn.id].id)} has been unlocked!`;
          openTooltip();
          updateItemBag();
    ...
    if (itemToUse != undefined) {
      ...
      if (itemToUse == item.abilityPatch.id) {
        div.addEventListener("click", (e) => {
          ...
          document.getElementById("tooltipTop").style.display = "none";
          document.getElementById("tooltipMid").innerHTML =
            `<div class="genetics-overview-tags"><div style="filter:hue-rotate(0deg)">★ ${format(newAbility)}</div></div>`;
          if (ability[newAbility].rarity === 2)
            document.getElementById("tooltipMid").innerHTML =
              `<div class="genetics-overview-tags"><div style="filter:hue-rotate(100deg)">★ ${format(newAbility)} (Uncommon!)</div></div>`;
          if (ability[newAbility].rarity === 3)
            document.getElementById("tooltipMid").innerHTML =
              `<div class="genetics-overview-tags"><div style="filter:hue-rotate(200deg)">★ ${format(newAbility)} (Rare!)</div></div>`;
    ...
    if (dexTeamSelect != undefined) {
      //called when switching team member

      //this prevents already equipped pokemon to be equipped again
      if (saved.previewTeams[saved.currentPreviewTeam].slot1.pkmn == pkmn[i].id)
        continue;
      if (saved.previewTeams[saved.currentPreviewTeam].slot2.pkmn == pkmn[i].id)
        continue;
      if (saved.previewTeams[saved.currentPreviewTeam].slot3.pkmn == pkmn[i].id)
        continue;
      if (saved.previewTeams[saved.currentPreviewTeam].slot4.pkmn == pkmn[i].id)
        continue;
      if (saved.previewTeams[saved.currentPreviewTeam].slot5.pkmn == pkmn[i].id)
        continue;
      if (saved.previewTeams[saved.currentPreviewTeam].slot6.pkmn == pkmn[i].id)
        continue;

      div.addEventListener("click", (e) => {
        saved.previewTeams[saved.currentPreviewTeam][dexTeamSelect].pkmn =
          pkmn[i].id;
        updatePreviewTeam();
  ...
  document.getElementById(`pokedex-total`).innerHTML =
    `Caught: ${gotPokemon}/${totalPokemon}`;
  if (gotPokemon == totalPokemon) {
    document.getElementById(`pokedex-total`).style.background =
      "rgba(187, 146, 85, 1)";
    document.getElementById(`pokedex-total`).innerHTML =
      `Caught: ${gotPokemon}/${totalPokemon} <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 48 48"><defs><mask id="SVGIz3eBe9X"><g fill="none" stroke="#fff" stroke-linecap="round" stroke-linejoin="round" stroke-width="4"><path d="M8 44h8m-4 0V4"/><path fill="#555555" d="M40 6H12v16h28l-4-8z"/></g></mask></defs><path fill="currentColor" d="M0 0h48v48H0z" mask="url(#SVGIz3eBe9X)"/></svg>`;
  } else
    document.getElementById(`pokedex-total`).style.background =
      "rgba(91, 114, 163, 1)";

  document.getElementById(`pokedex-total`).style.display = "flex";
  if (document.getElementById(`pokedex-filter-level`).value !== "all")
    document.getElementById(`pokedex-total`).style.display = "none";
  if (document.getElementById(`pokedex-filter-tag`).value !== "all")
    document.getElementById(`pokedex-total`).style.display = "none";
  if (document.getElementById(`pokedex-filter-ability`).value !== "all")
    document.getElementById(`pokedex-total`).style.display = "none";
  if (document.getElementById(`pokedex-filter-evolution`).value !== "all")
    document.getElementById(`pokedex-total`).style.display = "none";
  if (document.getElementById("pokedex-search").value != "")
    document.getElementById(`pokedex-total`).style.display = "none";
}
...
function exitTmTeaching(mod) {
  //what a fucking disgrace of a code i wrote here
  ...
  if (dexTeamSelect !== undefined && mod == "remove") {
    ...
    document.getElementById(`pokedex-menu`).style.display = "none";
    document.getElementById(`pokedex-menu`).style.zIndex = "30";
    saved.previewTeams[saved.currentPreviewTeam][dexTeamSelect].item =
      undefined;
    saved.previewTeams[saved.currentPreviewTeam][dexTeamSelect].pkmn =
      undefined;
    dexTeamSelect = undefined;
...
function switchMenu(id) {
  ...
  if (
    /vs|items|team|dex|dictionary|guide/.test(id) &&
    saved.tutorialStep != "none"
  ) {
    document.getElementById("tooltipTop").style.display = `none`;
    document.getElementById("tooltipTitle").style.display = `none`;
  ...
  if (id == "team")
    document.getElementById(`auto-build-button`).style.display = "none";
  else document.getElementById(`auto-build-button`).style.display = "flex";

  if (id == "team")
    document.getElementById(`pkmn-team-return`).style.display = "none";
  else document.getElementById(`pkmn-team-return`).style.display = "flex";

  if (id == "dex")
    document.getElementById(`pokedex-filters`).style.paddingTop = "6.5rem";
  ...
  if (saved.tutorial && saved.tutorialStep === "intro") {
    saved.tutorialStep = "travel";
    openTutorial();
  }
...
function updateItemBag() {
  ...
  for (const i in item) {
    ...
    if (
      item[i].rotation &&
      !Array.isArray(item[i].rotation) &&
      item[i].rotation !== rotationEventCurrent
    )
      item[i].got = 0;
    if (
      item[i].rotation &&
      Array.isArray(item[i].rotation) &&
      !item[i].rotation.includes(rotationEventCurrent)
    )
      item[i].got = 0;
    ...
    div.dataset.item = i;
    if (item[i].type !== "tm")
      div.innerHTML = `<img src="img/items/${i}.png"> <span class="item-list-name">${format(i)}</span> <span>x${item[i].got}</span>`;
    if (item[i].move && move[item[i].move])
      div.innerHTML = `<img src="img/items/tm${format(move[item[i].move].type)}.png"> <span class="item-list-name">${format(i)} <strong style="opacity:0.6; font-weight:200; white-space:nowrap; font-size:0.9rem; margin-left:0.2rem"> (${move[item[i].move].power} BP, ${format(move[item[i].move].split).slice(0, 3)})</strong> </span>  <span>x${item[i].got}</span>`;
    ...
    if (item[i].type == "tm" && dexTeamSelect == undefined) {
      div.addEventListener("click", (e) => {
        ...
        document.getElementById("pokedex-filters-title").style.display = "flex";
        document.getElementById("pokedex-filters-cancel").style.display =
          "flex";
        document.getElementById("pokedex-filters-title").innerHTML =
          `Select a Pokemon to teach ${format(item[i].move)}`;
        document.getElementById("menu-button-parent").style.display = "none";
        document.getElementById(`item-menu`).style.display = "none";
      });
    }
    ...
    if (item[i].evo && dexTeamSelect == undefined) {
      div.addEventListener("click", (e) => {
        ...
        document.getElementById("pokedex-filters-cancel").style.display =
          "flex";
        document.getElementById("pokedex-filters-title").innerHTML =
          `Select a Pokemon to use the ${format(i)}`;
        document.getElementById("menu-button-parent").style.display = "none";
        document.getElementById(`item-menu`).style.display = "none";
      });
    }
    ...
    if (dexTeamSelect != undefined) {
      ...
      if (saved.previewTeams[saved.currentPreviewTeam].slot1.item == i)
        continue;
      if (saved.previewTeams[saved.currentPreviewTeam].slot2.item == i)
        continue;
      if (saved.previewTeams[saved.currentPreviewTeam].slot3.item == i)
        continue;
      if (saved.previewTeams[saved.currentPreviewTeam].slot4.item == i)
        continue;
      if (saved.previewTeams[saved.currentPreviewTeam].slot5.item == i)
        continue;
      if (saved.previewTeams[saved.currentPreviewTeam].slot6.item == i)
        continue;
    ...
    if (geneticItemSelect == true) {
      ...
      if (i == item.destinyKnot.id && currentGeneticsCompatibility <= 1)
        continue;
      if (
        i == item.lockCapsule.id &&
        (currentGeneticsCompatibility <= 1 ||
          pkmn[saved.geneticSample].movepool.length < 5)
      )
        continue;
...
function updateVS() {
  ...
  document.getElementById("vs-menu-header").innerHTML = `
    ...
    `;
  document.getElementById("vs-menu-header").style.backgroundImage =
    "url(img/bg/gym.png)";
  ...
  for (const i in areas) {
    ...
    if (!firstOne) {
      ...
      document.getElementById(`trainer-image-${areas[i].name}`).style.filter =
        "brightness(0)";
  ...
  if (document.getElementById(`vs-listing`).innerHTML == "")
    document.getElementById(`vs-listing`).innerHTML =
      `<div style="display:flex; flex-direction:column; justify-content:center; align-items:center; background:#ECDEB7; border-radius:0.3rem; height:15rem; width:15rem; text-align:center"><img src="img/pkmn/sprite/pikachuRockstar.png">All trainers defeated!<br><span style="font-size:0.9rem; opacity:0.7">How about the Battle Frontier?</span></div>`;
}
...
function resetSpiralingTower() {
  ...
  if (saved.lastTowerRotation != rotationEventCurrent) {
    saved.lastTowerRotation = rotationEventCurrent;
  }

  saved.spiralRewardsClaimed = 0;
  saved.maxSpiralFloor = 1;
  saved.currentSpiralingType = arrayPick([
    `normal`,
    `fire`,
    `water`,
    `grass`,
...
function createFrontierTrainers() {
  ...
  if (saved.lastFrontierRotation == rotationWildCurrent) return;
  if (saved.lastFrontierRotation != rotationWildCurrent) {
    saved.lastFrontierRotation = rotationWildCurrent;
  }
  ...
  for (const i in areas) {
    ...
    if (areas[i].tier == 1)
      areas[i].reward = [
        ...
        arrayPick(exclusiveFrontierPkmn),
      ];
    if (areas[i].tier == 2)
      areas[i].reward = [
        item.goldenBottleCap,
    ...
    let pkmn1 = pkmn[randomDivisionPkmn(divisionToUse, areas[i].typing)];
    let pkmn2 =
      pkmn[randomDivisionPkmn(divisionToUse, areas[i].typing, [pkmn1?.id])];
    let pkmn3 =
      pkmn[
        randomDivisionPkmn(divisionToUse, areas[i].typing, [
          pkmn1?.id,
          pkmn2?.id,
        ])
      ];
    ...
    if (pkmn1 == undefined)
      pkmn1 = pkmn[randomDivisionPkmn(divisionToUse, areas[i].typing)];
    if (pkmn2 == undefined)
      pkmn2 = pkmn[randomDivisionPkmn(divisionToUse, areas[i].typing, [pkmn1])];
    if (pkmn3 == undefined)
      pkmn3 =
        pkmn[
          randomDivisionPkmn(divisionToUse, areas[i].typing, [pkmn1, pkmn2])
        ];
    if (pkmn4 == undefined)
      pkmn4 =
        pkmn[
...
function randomDivisionPkmn(division, type, exclude, seed, mod) {
  ...
  for (const i in pkmn) {
    ...
    if (mod == "training") {
      if (
        typeEffectiveness([pkmn[i].type[1]], pkmn[saved.trainingPokemon].type) >
        1
      )
        continue;
      if (
        typeEffectiveness([pkmn[i].type[0]], pkmn[saved.trainingPokemon].type) >
        1
      )
        continue;
...
function updateFrontier() {
  ...
  let divisionText = ``;
  if (rotationFrontierCurrent == 1)
    divisionText = `<span style="font-size: 1.5rem; padding:0">${cupsvg}Little Cup${cupsvg}</span><div>${returnDivisionLetter("C")} division and below only</div>`;
  if (rotationFrontierCurrent == 2)
    divisionText = `<span style="font-size: 1.5rem; padding:0">${cupsvg}Great League${cupsvg}</span><div>${returnDivisionLetter("B")} division and below only</div>`;
  if (rotationFrontierCurrent == 3)
    divisionText = `<span style="font-size: 1.5rem; padding:0">${cupsvg}Ultra League${cupsvg}</span><div>${returnDivisionLetter("A")} division and below only</div>`;
  if (rotationFrontierCurrent == 4)
    divisionText = `<span style="font-size: 1.5rem; padding:0">${cupsvg}Master League${cupsvg}</span><div>${returnDivisionLetter("S")} division and below only</div>`;

  document.getElementById(`frontier-listing`).innerHTML =
    `<div class="frontier-league">${divisionText}</div>`;
...
function updateWildBuffs() {
  ...
  if (saved.weather !== undefined && saved.weatherTimer > 0) {
    ...
    if (saved.weather == "sunny") {
      div.style.filter = `hue-rotate(130deg)`;
    ...
    if (saved.weather == "rainy") {
      div.style.filter = `hue-rotate(-20deg)`;
      weatherIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="8" height="8" viewBox="0 0 8 8"><path fill="currentColor" d="M4.5 0C3.29 0 2.23.86 2 2C.9 2 0 2.9 0 4c0 .53.2.99.53 1.34c.26-.22.6-.34.97-.34c.2 0 .39.05.56.13C2.23 4.49 2.8 4 3.5 4c.69 0 1.27.49 1.44 1.13c.17-.07.36-.13.56-.13c.63 0 1.15.39 1.38.94c.64-.17 1.13-.75 1.13-1.44c0-.65-.42-1.29-1-1.5v-.5A2.5 2.5 0 0 0 4.51 0zM3.34 5a.5.5 0 0 0-.34.5v2a.5.5 0 1 0 1 0v-2a.5.5 0 0 0-.59-.5h-.06zm-2 1a.5.5 0 0 0-.34.5v1a.5.5 0 1 0 1 0v-1a.5.5 0 0 0-.59-.5h-.06zm4 0a.5.5 0 0 0-.34.5v1a.5.5 0 1 0 1 0v-1a.5.5 0 0 0-.59-.5h-.06z"/></svg>`;
    }

    if (saved.weather == "sandstorm") {
      div.style.filter = `hue-rotate(140deg)`;
      weatherIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M5.268 5H4v1h3a2 2 0 1 0-1.732-1M3 6H2V5h1zm8.085 1H9v1h3.5a1.5 1.5 0 1 0-1.415-1M8 7v1H2V7zM6 9v1H2V9zm3.5 1H7V9h4.5a2.5 2.5 0 1 1-2 1" clip-rule="evenodd"/></svg>`;
    }
    if (saved.weather == "hail") {
      div.style.filter = `hue-rotate(-50deg)`;
    ...
    if (saved.weather == "grassyTerrain") {
      div.style.filter = `hue-rotate(220deg)`;
...
function changeWeather(id, mod) {
  if (saved.weatherCooldown <= 0 || mod == "force") {
    ...
    if (id == "rainy" && team[exploreActiveMember].item == item.dampRock.id)
      weatherTurns += item.dampRock.power();
    if (id == "sunny" && team[exploreActiveMember].item == item.heatRock.id)
      weatherTurns += item.heatRock.power();
    if (id == "hail" && team[exploreActiveMember].item == item.icyRock.id)
      weatherTurns += item.icyRock.power();
    if (
      id == "sandstorm" &&
      team[exploreActiveMember].item == item.smoothRock.id
    )
      weatherTurns += item.smoothRock.power();
...
function updateTeamBuffs() {
  if (document.getElementById("team-member-slot1-buff-list"))
    document.getElementById("team-member-slot1-buff-list").innerHTML = "";
  if (document.getElementById("team-member-slot2-buff-list"))
    document.getElementById("team-member-slot2-buff-list").innerHTML = "";
  if (document.getElementById("team-member-slot3-buff-list"))
    document.getElementById("team-member-slot3-buff-list").innerHTML = "";
  if (document.getElementById("team-member-slot4-buff-list"))
    document.getElementById("team-member-slot4-buff-list").innerHTML = "";
  if (document.getElementById("team-member-slot5-buff-list"))
    document.getElementById("team-member-slot5-buff-list").innerHTML = "";
  if (document.getElementById("team-member-slot6-buff-list"))
    document.getElementById("team-member-slot6-buff-list").innerHTML = "";
  ...
  for (const slot in team) {
    ...
    for (const i in team[slot].buffs) {
      ...
      if (testAbility(slot, ability.insomnia.id) && i == "sleep")
        team[slot].buffs.sleep = 0;
      if (testAbility(slot, ability.immunity.id) && i == "poisoned")
        team[slot].buffs.poisoned = 0;
      if (testAbility(slot, ability.limber.id) && i == "paralysis")
        team[slot].buffs.paralysis = 0;
      if (testAbility(slot, ability.ownTempo.id) && i == "confused")
        team[slot].buffs.confused = 0;
      if (testAbility(slot, ability.magmaArmor.id) && i == "freeze")
        team[slot].buffs.freeze = 0;
      if (testAbility(slot, ability.waterVeil.id) && i == "burn")
        team[slot].buffs.burn = 0;

      if (
        testAbility(slot, ability.hyperCutter.id) &&
        (i == "atkdown1" || i == "atkdown2")
      ) {
        team[slot].buffs.atkdown1 = 0;
        team[slot].buffs.atkdown2 = 0;
        continue;
      }
      if (
        testAbility(slot, ability.bigPecks.id) &&
        (i == "defdown1" || i == "defdown2")
      ) {
        team[slot].buffs.defdown1 = 0;
        team[slot].buffs.defdown2 = 0;
        continue;
      }

      if (
        testAbility(slot, ability.fullMetalBody.id) &&
        /atkdown1|atkdown2|defdown1|defdown2|stakdown1|stakdown2|sdefdown1|sdefdown2|spedown1|spedown2/.test(
          i,
        )
      )
        team[slot].buffs[i] = 0;
      ...
      div.style.filter = `hue-rotate(${formatBuffs(i, "hue")}deg)`;
      div.innerHTML = formatBuffs(i);
      if (document.getElementById(`team-member-${slot}-buff-list`))
        document
          .getElementById(`team-member-${slot}-buff-list`)
          .appendChild(div);
    }
  }
}
...
function moveBuff(target, buff, mod) {
  ...
  if (target === "wild" && mod == "team") {
    if (
      team[exploreActiveMember].item == item.lightClay.id &&
      /atkup1|atkup2|defup1|defup2|stakup1|stakup2|sdefup1|sdefup2|speup1|speup2/.test(
        buff,
      )
    )
      affectedTurns++;
    if (
      team[exploreActiveMember].item == item.mentalHerb.id &&
      /atkdown1|atkdown2|defdown1|defdown2|stakdown1|stakdown2|sdefdown1|sdefdown2|spedown1|spedown2burn|freeze|confused|paralysis|poisoned|sleep/.test(
        buff,
      )
    )
      affectedTurns--;
  ...
  if (
    (target === "player" && mod == "self") ||
    (mod == undefined && target === "wild")
  ) {
    //if (buff == "paralysis" && pkmn[saved.currentPkmn].type.includes("electric")) return
    if (
      /burn|freeze|confused|paralysis|poisoned|sleep/.test(buff) &&
      (wildBuffs.burn > 0 ||
        wildBuffs.freeze > 0 ||
        wildBuffs.confused > 0 ||
        wildBuffs.paralysis > 0 ||
        wildBuffs.poisoned > 0 ||
        wildBuffs.sleep > 0)
    )
      return;
    ...
    if (
      testAbility(`active`, ability.imposter.id) &&
      /atkup1|atkup2|defup1|defup2|stakup1|stakup2|sdefup1|sdefup2|speup1|speup2/.test(
        buff,
      )
    )
  ...
  if (
    (target === "wild" && mod == "self") ||
    (mod == undefined && target === "player")
  ) {
    ...
    if (
      ...
      (team[exploreActiveMember].buffs?.burn > 0 ||
        team[exploreActiveMember].buffs?.freeze > 0 ||
        team[exploreActiveMember].buffs?.confused > 0 ||
        team[exploreActiveMember].buffs?.paralysis > 0 ||
        team[exploreActiveMember].buffs?.poisoned > 0 ||
        team[exploreActiveMember].buffs?.sleep > 0)
    )
      return;
    ...
    if (
      testAbility(`active`, ability.synchronize.id) &&
      /burn|freeze|confused|paralysis|poisoned|sleep/.test(buff)
    ) {
      moveBuff("wild", buff);
      updateWildBuffs();
    ...
    if (
      testAbility(`active`, ability.wonderSkin.id) &&
      /burn|freeze|confused|paralysis|poisoned|sleep/.test(buff) &&
      rng(0.5)
    )
      return;
    ...
    if (
      testAbility(`active`, ability.hydratation.id) &&
      saved.weather == "rainy" &&
      saved.weatherTimer > 0 &&
      /burn|freeze|confused|paralysis|poisoned|sleep/.test(buff)
    ) {
    ...
    if (
      testAbility(`active`, ability.sandVeil.id) &&
      saved.weather == "sandstorm" &&
      saved.weatherTimer > 0 &&
      /burn|freeze|confused|paralysis|poisoned|sleep/.test(buff)
    ) {
    ...
    if (
      testAbility(`active`, ability.snowCloak.id) &&
      saved.weather == "hail" &&
      saved.weatherTimer > 0 &&
      /burn|freeze|confused|paralysis|poisoned|sleep/.test(buff)
    ) {
    ...
    if (
      testAbility(`active`, ability.grassyPelt.id) &&
      saved.weather == "rainy" &&
      saved.weatherTimer > 0 &&
      /burn|freeze|confused|paralysis|poisoned|sleep|defdown1|defdown2|atkdown1|atkdown2|sdefdown1|sdefdown2|satkdown1|satkdown2|spedown1|spedown2/.test(
        buff,
      )
    ) {
      return;
    ...
    if (
      testAbility(`active`, ability.sandyPelt.id) &&
      saved.weather == "sunny" &&
      saved.weatherTimer > 0 &&
      /burn|freeze|confused|paralysis|poisoned|sleep|defdown1|defdown2|atkdown1|atkdown2|sdefdown1|sdefdown2|satkdown1|satkdown2|spedown1|spedown2/.test(
        buff,
    ...
    if (
      testAbility(`active`, ability.moistPelt.id) &&
      saved.weather == "sandstorm" &&
      saved.weatherTimer > 0 &&
      /burn|freeze|confused|paralysis|poisoned|sleep|defdown1|defdown2|atkdown1|atkdown2|sdefdown1|sdefdown2|satkdown1|satkdown2|spedown1|spedown2/.test(
        buff,
    ...
    if (
      testAbility(`active`, ability.fieryPelt.id) &&
      saved.weather == "hail" &&
      saved.weatherTimer > 0 &&
      /burn|freeze|confused|paralysis|poisoned|sleep|defdown1|defdown2|atkdown1|atkdown2|sdefdown1|sdefdown2|satkdown1|satkdown2|spedown1|spedown2/.test(
        buff,
    ...
    if (
      testAbility(`active`, ability.pixiePelt.id) &&
      saved.weather == "foggy" &&
      saved.weatherTimer > 0 &&
      /burn|freeze|confused|paralysis|poisoned|sleep|defdown1|defdown2|atkdown1|atkdown2|sdefdown1|sdefdown2|satkdown1|satkdown2|spedown1|spedown2/.test(
        buff,
    ...
    if (
      testAbility(`active`, ability.contrary.id) &&
      typeof buff === "string" &&
      (buff.includes("down") || buff.includes("up"))
    ) {
...
function switchShiny() {
  if (
    pkmn[currentEditedPkmn].shiny &&
    pkmn[currentEditedPkmn].shinyDisabled !== true
  ) {
    pkmn[currentEditedPkmn].shinyDisabled = true;
    document.getElementById("pkmn-editor-sprite").src =
      `img/pkmn/sprite/${currentEditedPkmn}.png`;
    return;
  }
  ...
  if (
    ...
    document.getElementById("pkmn-editor-sprite").src =
      `img/pkmn/shiny/${currentEditedPkmn}.png`;
    return;
  }
}
...
function returnDivisionLetter(division) {
  if (division === "D")
    return `<strong style="color:#DAA867" class="tier-letter">D</strong>`;
  if (division === "C")
    return `<strong style="color:#73C46A" class="tier-letter">C</strong>`;
  if (division === "B") return `<strong class="tier-letter">B</strong>`;
  if (division === "A")
    return `<strong style="color:#EB535D" class="tier-letter">A</strong>`;
...
document
  .getElementById("genetics-sample-div")
...
document.getElementById("genetics-start").addEventListener("click", (e) => {
  ...
  if (saved.geneticHost == undefined || saved.geneticSample == undefined)
    return;
...
function assignPokerus() {
  ...
  if (saved.lastPokerusReset != rotationWildCurrent) {
    ...
    for (const i in pkmn) {
      ...
      if (pkmn[i].pokerus) {
        pkmn[i].pokerus = undefined;
        pkmn[i].tagPokerus = undefined;
      }
    }
...
function setGeneticMenu(mod, item) {
  ...
  if (saved.geneticHost != undefined) {
    document.getElementById("genetics-host-div").innerHTML =
      `<img class="sprite-trim" style="z-index: 1; transform-origin: bottom; margin-top: auto; position: absolute; bottom: -4.5rem;" id="genetics-host" src="img/pkmn/sprite/${hostPkmn.id}.png">
<img style="animation: none; position: absolute; opacity: 0.4; width: 4.5rem; z-index: 0; transform: translateY(0.5rem); image-rendering: initial;" src="img/resources/pkmn-shadow.png">`;
    if (pkmn[saved.geneticHost].shiny) {
      document.getElementById("genetics-host").src =
        `img/pkmn/shiny/${saved.geneticHost}.png`;
    }
  } else {
    document.getElementById("genetics-host-div").innerHTML =
      `<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 1024 1024"><path fill="currentColor" d="M512 0C229.232 0 0 229.232 0 512c0 282.784 229.232 512 512 512c282.784 0 512-229.216 512-512C1024 229.232 794.784 0 512 0m0 961.008c-247.024 0-448-201.984-448-449.01c0-247.024 200.976-448 448-448s448 200.977 448 448s-200.976 449.01-448 449.01M736 480H544V288c0-17.664-14.336-32-32-32s-32 14.336-32 32v192H288c-17.664 0-32 14.336-32 32s14.336 32 32 32h192v192c0 17.664 14.336 32 32 32s32-14.336 32-32V544h192c17.664 0 32-14.336 32-32s-14.336-32-32-32"/></svg>`;
  }

  if (saved.geneticSample != undefined) {
    document.getElementById("genetics-sample-div").innerHTML =
      `<img class="sprite-trim" style="animation: none; z-index: 1;" id="genetics-sample" src="img/pkmn/sprite/${samplePkmn.id}.png">`;
    if (pkmn[saved.geneticSample].shiny) {
      document.getElementById("genetics-sample").src =
        `img/pkmn/shiny/${samplePkmn.id}.png`;
    }
  } else {
    document.getElementById("genetics-sample-div").innerHTML =
  ...
  if (saved.geneticHost !== undefined) {
    document.getElementById("genetics-host-div").dataset.pkmnEditor =
      saved.geneticHost;
  } else {
    delete document.getElementById("genetics-host-div").dataset.pkmnEditor;
  }
  if (saved.geneticSample !== undefined) {
    document.getElementById("genetics-sample-div").dataset.pkmnEditor =
      saved.geneticSample;
  } else {
    delete document.getElementById("genetics-host-div").dataset.pkmnEditor;
  }
  ...
  if (saved.geneticHost == undefined || saved.geneticSample == undefined)
    ...
    const sharedType = hostPkmn.type.filter((type) =>
      samplePkmn.type.includes(type),
    ).length;
    ...
    if (pkmn[saved.geneticHost].pokerus || saved.geneticPokerus == true)
      compability++;
    if (pkmn[saved.geneticHost].pokerus || saved.geneticPokerus == true)
      document.getElementById("pokerus-warning").style.display = "flex";
  ...
  document.getElementById("genetics-bar-compatibility").style.width =
    `${((compability - 1) / 3) * 100}%`;

  document.getElementById("genetics-bar-power").style.width =
    `${(powerCost / 12) * 100}%`;

  if (powerCost >= 8)
    document.getElementById("genetics-bar-power").style.backgroundColor =
      `coral`;
  else {
    document.getElementById("genetics-bar-power").style.backgroundColor =
      `rgb(229, 143, 255)`;
  ...
  if (powerCost >= 8)
    document.getElementById("genetics-warning").style.display = "flex";
  if (powerCost == 8)
    document.getElementById("genetics-warning").innerHTML =
  ...
  document.getElementById("genetics-data-shiny").innerHTML =
    `${(shinyChance * 100).toFixed(0)}%`;

  let moveChance = 0.05;
  if (compability == 2) moveChance = 0.3;
  if (compability == 3) moveChance = 0.5;
  if (compability == 4) moveChance = 0.5;
  if (compability == 0) moveChance = 0;

  document.getElementById("genetics-data-moves").innerHTML =
    `${(moveChance * 100).toFixed(0)}%`;
  ...
  document.getElementById("genetics-data-hp").innerHTML =
    `${(ivChance * 100).toFixed(0)}%`;
  document.getElementById("genetics-data-atk").innerHTML =
    `${(ivChance * 100).toFixed(0)}%`;
  document.getElementById("genetics-data-def").innerHTML =
    `${(ivChance * 100).toFixed(0)}%`;
  document.getElementById("genetics-data-satk").innerHTML =
    `${(ivChance * 100).toFixed(0)}%`;
  document.getElementById("genetics-data-sdef").innerHTML =
    `${(ivChance * 100).toFixed(0)}%`;
  document.getElementById("genetics-data-spe").innerHTML =
    `${(ivChance * 100).toFixed(0)}%`;
  ...
  if (saved.geneticOperation !== undefined) {
    ...
    document.getElementById("genetics-arrow").style.animation =
      "genetics-arrow 2s infinite";
    document.getElementById("genetics-host").style.animation =
      "pkmn-active 0.5s infinite";
    document.getElementById("genetics-progress-time").innerHTML = returnHMS(
      saved.geneticOperation,
    );
    document.getElementById("genetics-progress-bar").style.width =
      `${100 - (saved.geneticOperation / saved.geneticOperationTotal) * 100}%`;
  } else {
    document.getElementById("genetics-start").textContent = "Start";
    document.getElementById("genetics-start").style.color =
      "rgb(189, 123, 219)";
    document.getElementById("genetics-start").style.outlineColor =
      "rgb(189, 123, 219)";
    document.getElementById("genetics-progress").style.display = "none";
    document.getElementById("genetics-arrow").style.animation = "none";
    if (document.getElementById("genetics-host"))
      document.getElementById("genetics-host").style.animation = "none";
  }
  ...
  if (mod === "end") {
    ...
    if (rng(shinyChance)) {
      pkmn[saved.geneticHost].shiny = true;
      summaryTags += `<div style="filter:hue-rotate(100deg)">✦ Shiny Mutation!</div>`;
    }
    ...
    if (item == "destinyKnot") {
      ...
      if (item == "everstone") {
        ...
        summaryTags += `<div style="filter:hue-rotate(-50deg)">★ New ability: ${format(newAbility)}!</div>`;
      }
    }

    pkmn[saved.geneticHost].movepool = [];
    if (pkmn[saved.geneticHost].moves.slot1 !== undefined)
      pkmn[saved.geneticHost].movepool.push(
        pkmn[saved.geneticHost].moves.slot1,
      );
    if (pkmn[saved.geneticHost].moves.slot2 !== undefined)
      pkmn[saved.geneticHost].movepool.push(
        pkmn[saved.geneticHost].moves.slot2,
      );
    if (pkmn[saved.geneticHost].moves.slot3 !== undefined)
      pkmn[saved.geneticHost].movepool.push(
        pkmn[saved.geneticHost].moves.slot3,
      );
    if (pkmn[saved.geneticHost].moves.slot4 !== undefined)
      pkmn[saved.geneticHost].movepool.push(
        pkmn[saved.geneticHost].moves.slot4,
      );
    ...
    pkmn[saved.geneticSample].movepool.forEach((moveID) => {
      if (
        rng(moveChance) &&
        !pkmn[saved.geneticHost].movepool.includes(moveID) &&
        move[moveID].moveset !== undefined
      ) {
    ...
    if (item == `lockCapsule`) {
      ...
      if (
        pkmn[saved.geneticSample].moves.slot1 !== undefined &&
        !pkmn[saved.geneticHost].movepool.includes(
          pkmn[saved.geneticSample].moves.slot1,
        ) &&
        move[pkmn[saved.geneticSample].moves.slot1].moveset !== undefined
      )
        pkmn[saved.geneticHost].movepool.push(
          pkmn[saved.geneticSample].moves.slot1,
        );
      if (
        pkmn[saved.geneticSample].moves.slot2 !== undefined &&
        !pkmn[saved.geneticHost].movepool.includes(
          pkmn[saved.geneticSample].moves.slot2,
        ) &&
        move[pkmn[saved.geneticSample].moves.slot2].moveset !== undefined
      )
        pkmn[saved.geneticHost].movepool.push(
          pkmn[saved.geneticSample].moves.slot2,
        );
      if (
        pkmn[saved.geneticSample].moves.slot3 !== undefined &&
        !pkmn[saved.geneticHost].movepool.includes(
          pkmn[saved.geneticSample].moves.slot3,
        ) &&
        move[pkmn[saved.geneticSample].moves.slot3].moveset !== undefined
      )
        pkmn[saved.geneticHost].movepool.push(
          pkmn[saved.geneticSample].moves.slot3,
        );
      if (
        pkmn[saved.geneticSample].moves.slot4 !== undefined &&
        !pkmn[saved.geneticHost].movepool.includes(
          pkmn[saved.geneticSample].moves.slot4,
        ) &&
        move[pkmn[saved.geneticSample].moves.slot4].moveset !== undefined
      )
        pkmn[saved.geneticHost].movepool.push(
          pkmn[saved.geneticSample].moves.slot4,
        );
      //delete sample pool
      if (move[pkmn[saved.geneticSample].moves.slot1].moveset !== undefined)
        pkmn[saved.geneticSample].movepool.splice(
          pkmn[saved.geneticSample].movepool.indexOf(
            pkmn[saved.geneticSample].moves.slot1,
          ),
          1,
        );
      if (move[pkmn[saved.geneticSample].moves.slot2].moveset !== undefined)
        pkmn[saved.geneticSample].movepool.splice(
          pkmn[saved.geneticSample].movepool.indexOf(
            pkmn[saved.geneticSample].moves.slot2,
          ),
      ...
      if (move[pkmn[saved.geneticSample].moves.slot3].moveset !== undefined)
        pkmn[saved.geneticSample].movepool.splice(
          pkmn[saved.geneticSample].movepool.indexOf(
            pkmn[saved.geneticSample].moves.slot3,
          ),
      ...
      if (move[pkmn[saved.geneticSample].moves.slot4].moveset !== undefined)
        pkmn[saved.geneticSample].movepool.splice(
          pkmn[saved.geneticSample].movepool.indexOf(
            pkmn[saved.geneticSample].moves.slot4,
          ),
      ...
      for (const e of pkmn[saved.geneticSample].movepool) {
        if (
          pkmn[saved.geneticSample].moves.slot1 == undefined &&
          pkmn[saved.geneticSample].moves.slot1 != e &&
          pkmn[saved.geneticSample].moves.slot2 != e &&
          pkmn[saved.geneticSample].moves.slot3 != e &&
          pkmn[saved.geneticSample].moves.slot4 != e
        )
          pkmn[saved.geneticSample].moves.slot1 = e;
        if (
          pkmn[saved.geneticSample].moves.slot2 == undefined &&
    ...
    if (
      rng(ivChanceHp) &&
      pkmn[saved.geneticHost].ivs.hp <
        Math.min(ivCap, pkmn[saved.geneticSample].ivs.hp)
    ) {
      pkmn[saved.geneticHost].ivs.hp = Math.min(
        ivCap,
        pkmn[saved.geneticSample].ivs.hp,
      );
      summaryTags += `<div style="filter:hue-rotate(200deg)">❖ HP Iv's inherited!</div>`;
    }
    if (
      rng(ivChanceAtk) &&
      pkmn[saved.geneticHost].ivs.atk <
        Math.min(ivCap, pkmn[saved.geneticSample].ivs.atk)
    ) {
      pkmn[saved.geneticHost].ivs.atk = Math.min(
        ivCap,
        pkmn[saved.geneticSample].ivs.atk,
      );
      summaryTags += `<div style="filter:hue-rotate(200deg)">❖ Attack Iv's inherited!</div>`;
    }
    if (
      rng(ivChanceDef) &&
      pkmn[saved.geneticHost].ivs.def <
        Math.min(ivCap, pkmn[saved.geneticSample].ivs.def)
    ) {
      pkmn[saved.geneticHost].ivs.def = Math.min(
        ivCap,
        pkmn[saved.geneticSample].ivs.def,
      );
      summaryTags += `<div style="filter:hue-rotate(200deg)">❖ Defense Iv's inherited!</div>`;
    }
    if (
      rng(ivChanceSatk) &&
      pkmn[saved.geneticHost].ivs.satk <
        Math.min(ivCap, pkmn[saved.geneticSample].ivs.satk)
    ) {
      pkmn[saved.geneticHost].ivs.satk = Math.min(
        ivCap,
        pkmn[saved.geneticSample].ivs.satk,
      );
      summaryTags += `<div style="filter:hue-rotate(200deg)">❖ Special Attack Iv's inherited!</div>`;
    }
    if (
      rng(ivChanceSdef) &&
      pkmn[saved.geneticHost].ivs.sdef <
        Math.min(ivCap, pkmn[saved.geneticSample].ivs.sdef)
    ) {
      pkmn[saved.geneticHost].ivs.sdef = Math.min(
        ivCap,
        pkmn[saved.geneticSample].ivs.sdef,
      );
      summaryTags += `<div style="filter:hue-rotate(200deg)">❖ Special Defense Iv's inherited!</div>`;
    }
    if (
      rng(ivChanceSpe) &&
      pkmn[saved.geneticHost].ivs.spe <
        Math.min(ivCap, pkmn[saved.geneticSample].ivs.spe)
    ) {
      pkmn[saved.geneticHost].ivs.spe = Math.min(
        ivCap,
        pkmn[saved.geneticSample].ivs.spe,
      );
      summaryTags += `<div style="filter:hue-rotate(200deg)">❖ Speed Iv's inherited!</div>`;
    }
    ...
    for (const iv in pkmn[saved.geneticHost].ivs) {
      ...
      if (newIv > ivId) {
        ...
        if (iv === "hp")
          summaryTags += `<div style="filter:hue-rotate(250deg)">◆ HP Iv's increased!</div>`;
        if (iv === "atk")
          summaryTags += `<div style="filter:hue-rotate(250deg)">◆ Attack Iv's increased!</div>`;
        if (iv === "def")
          summaryTags += `<div style="filter:hue-rotate(250deg)">◆ Defense Iv's increased!</div>`;
        if (iv === "satk")
          summaryTags += `<div style="filter:hue-rotate(250deg)">◆ Special Attack Iv's increased!</div>`;
        if (iv === "sdef")
          summaryTags += `<div style="filter:hue-rotate(250deg)">◆ Special Defense Iv's increased!</div>`;
        if (iv === "spe")
          summaryTags += `<div style="filter:hue-rotate(250deg)">◆ Speed Iv's increased!</div>`;
      }
    }
    ...
    document.getElementById("tooltipTop").style.display = "none";
    document.getElementById("tooltipMid").innerHTML =
      `<div class="genetics-overview-tags" id="prevent-tooltip-exit">${summaryTags}</div>`;
    document.getElementById("tooltipBottom").innerHTML =
      `<div style="display:flex;justify-content:center;align-items:center; width:100%; cursor:help"><div class="area-preview" data-pkmn-editor="${saved.geneticHost}"><img   class="sprite-trim" src="img/pkmn/sprite/${saved.geneticHost}.png"> </div></div>`;
...
document
  .getElementById("training-sprite-div")
  .addEventListener("click", (e) => {
    document.getElementById(`pokedex-menu`).style.display = "flex";
    document.getElementById(`pokedex-menu`).style.zIndex = "200";
...
training.level = {
  ...
  condition: function () {
    if (
      pkmn[saved.trainingPokemon].level < 100 &&
      areas.vsEliteTrainerCynthia.defeated == true
    )
      return true;
  },
  effect: function () {
    for (let i = 0; i < 101; i++) {
      ...
      let learntMove = learnPkmnMove(
        pkmn[saved.trainingPokemon].id,
        pkmn[saved.trainingPokemon].level,
      );
      if (learntMove != undefined) {
        if (pkmn[saved.trainingPokemon].level % 7 === 0)
...
training.iv1 = {
  //disapears if you have more than x ivs
  name: `IV Training I`,
  ...
  effect: function () {
    ...
    while (points > 0) {
      const available = stats.filter(
        (s) => pkmn[i].ivs[s] < 6 && (increases[s] || 0) < cap,
      );
...
training.iv2 = {
  //doesnt appear until you have more than x ivs
  name: `IV Training II`,
...
training.hiddenAbility = {
  ...
  condition: function () {
    if (
      pkmn[saved.trainingPokemon].hiddenAbility &&
...
training.move = {
  //disapears if you have 20+ moves or smth
  name: `Move Training`,
  ...
  condition: function () {
    if (
      ...
      pkmn[saved.trainingPokemon].movepool.length < 20
    )
      return true;
  },
  effect: function () {
...
function setTrainingMenu() {
  if (saved.trainingPokemon != undefined) {
    document.getElementById("training-sprite-div").innerHTML =
      `<img class="sprite-trim" style="z-index: 1; transform-origin: bottom; margin-top: auto; position: absolute; bottom: 0rem;" id="genetics-host" src="img/pkmn/sprite/${saved.trainingPokemon}.png">`;
    if (pkmn[saved.trainingPokemon].shiny) {
      document.getElementById("genetics-host").src =
        `img/pkmn/shiny/${saved.trainingPokemon}.png`;
    }
    voidAnimation("training-sack", "train-sack 1.5s infinite");
  } else {
    document.getElementById("training-sprite-div").innerHTML =
      `<svg style="color:white; scale:2; opacity:0.8; margin-bottom:-2rem;" xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 48 48"><defs><mask id="SVGjidSMeWm"><g fill="none" stroke="#fff" stroke-linejoin="round" stroke-width="4"><path fill="#555555" d="M24 44c11.046 0 20-8.954 20-20S35.046 4 24 4S4 12.954 4 24s8.954 20 20 20Z"/><path stroke-linecap="round" d="M24 16v16m-8-8h16"/></g></mask></defs><path fill="currentColor" d="M0 0h48v48H0z" mask="url(#SVGjidSMeWm)"/></svg>`;
    voidAnimation("training-sack", "none");
  }

  if (saved.trainingPokemon !== undefined) {
    document.getElementById("training-sprite-div").dataset.pkmnEditor =
      saved.trainingPokemon;
  } else {
    delete document.getElementById("training-sprite-div").dataset.pkmnEditor;
  }
  ...
  for (const i in training) {
    ...
    if (training[i].condition && training[i].condition() != true)
    ...
    div.addEventListener("click", (e) => {
      ...
      for (const activeMoves in pkmn[saved.trainingPokemon].moves) {
        if (pkmn[saved.trainingPokemon].moves[activeMoves] == undefined)
          continue;
        if (move[pkmn[saved.trainingPokemon].moves[activeMoves]].restricted)
          restricedActive++;
      }
      ...
      if (restrictedError) {
        ...
        document.getElementById("tooltipMid").innerHTML =
          `The training Pokemon has multiple restricted moves (${restrictedIcon}) equipped!`;
        openTooltip();
        return;
      }
...
function battleSummary() {
  ...
  for (const i in team) {
    if (team[i].pkmn == undefined) continue;
    if (team[i].damageDealt == NaN || team[i].damageDealt == undefined)
      team[i].damageDealt = 0;
  ...
  indexedTeam.sort(
    (a, b) => (b.data.damageDealt || 0) - (a.data.damageDealt || 0),
  );
  ...
  for (const item of indexedTeam) {
    ...
    document.getElementById(`battle-summary-bar-${i}`).style.width =
      percent + "%";
    document.getElementById(`battle-summary-bar-${i}`).innerHTML =
      `<text>${Math.round(team[i].damageDealt).toLocaleString("es-ES")} (${percent.toFixed(0)}%)</text>`;
  }
  openTooltip();
}
...
function returnHighestStat(pokemon) {
  //ignores hp bst, used for beast boost etc
...
function testAbility(target, ability) {
  if (target == "active") {
    if (pkmn[team[exploreActiveMember].pkmn.id]?.ability == ability)
      return true;
    if (
      pkmn[team[exploreActiveMember].pkmn.id]?.hiddenAbility?.id == ability &&
      pkmn[team[exploreActiveMember].pkmn.id]?.hiddenAbilityUnlocked == true
    )
      return true;
  }
  ...
  if (/slot1|slot2|slot3|slot4|slot5|slot6/.test(target)) {
    ...
    if (
      pkmn[team[target].pkmn.id]?.hiddenAbility?.id == ability &&
      pkmn[team[target].pkmn.id]?.hiddenAbilityUnlocked == true
    )
      return true;
  }
...
function claimMysteryGift() {
  ...
  document.getElementById("tooltipTop").innerHTML =
    `<img src="img/pkmn/shiny/${mysteryGift.icon}.png">`;
  ...
  document.getElementById("tooltipBottom").innerHTML =
    `<span data-pkmn-editor=${mysteryGift.icon} id="mystery-claim-button"><img src="img/items/gift.png" style="scale:4; image-rendering:pixelated; padding: 3rem 0; cursor:help" 
    style="cursor:pointer; font-size:2rem" id="prevent-tooltip-exit"></span>`;
  openTooltip();

  document
    .getElementById("mystery-claim-button")
