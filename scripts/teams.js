...
document
  .getElementById("team-slot-selector")
  .addEventListener("change", (e) => {
    saved.currentPreviewTeam =
      document.getElementById("team-slot-selector").value;
    updatePreviewTeam();
  });
...
function editTeamName() {
  ...
  document
    .getElementById("team-name-field")
...
function changeTeamNames() {
  ...
  for (const option of select.options) {
    for (const team in saved.previewTeams) {
      ...
      if (saved.previewTeams[team].name !== undefined)
        option.text = saved.previewTeams[team].name;
    }
  }

  document.getElementById("team-slot-selector").text =
...
function injectPreviewTeam() {
  ...
  for (const i in currentTeam) {
    ...
    if (
      ...
      returnPkmnDivision(pkmn[currentTeam[i].pkmn]) != "C" &&
      returnPkmnDivision(pkmn[currentTeam[i].pkmn]) != "D"
    )
      frontierError = true;
    if (
      areas[saved.currentAreaBuffer]?.type == "frontier" &&
      rotationFrontierCurrent === 2 &&
      returnPkmnDivision(pkmn[currentTeam[i].pkmn]) != "B" &&
      returnPkmnDivision(pkmn[currentTeam[i].pkmn]) != "C" &&
      returnPkmnDivision(pkmn[currentTeam[i].pkmn]) != "D"
    )
      frontierError = true;
    if (
      ...
      returnPkmnDivision(pkmn[currentTeam[i].pkmn]) != "A" &&
      returnPkmnDivision(pkmn[currentTeam[i].pkmn]) != "B" &&
      returnPkmnDivision(pkmn[currentTeam[i].pkmn]) != "C" &&
      returnPkmnDivision(pkmn[currentTeam[i].pkmn]) != "D"
    )
  ...
  for (const i in currentTeam) {
    ...
    for (const activeMoves in pkmn[currentTeam[i].pkmn].moves) {
      if (pkmn[currentTeam[i].pkmn].moves[activeMoves] == undefined) continue;
      if (move[pkmn[currentTeam[i].pkmn].moves[activeMoves]].restricted)
        restricedActive++;
    }
  ...
  if (restrictedError) {
    ...
    document.getElementById("tooltipMid").innerHTML =
      `One or more Pokemon in the current team have multiple restricted moves (${restrictedIcon}) equipped`;
    openTooltip();
    return;
  }
  ...
  if (saved.tutorial && saved.tutorialStep === "moves") {
    saved.tutorialStep = "battle";
    openTutorial();
    item.mysteryEgg.got++;
    item.mysteryEgg.newItem++;
  }
  ...
  for (const i in team) {
    ...
    if (saved.currentArea == areas.training.id) {
      ...
      pkmn[team[i].pkmn.id].playerHp =
        (100 +
          returnDivisionStars(pkmn[team[i].pkmn.id]) *
            30 *
            Math.pow(1.05, pkmn[team[i].pkmn.id].ivs.hp) *
            (1 + pkmn[team[i].pkmn.id].level * 0.2)) *
        hpMultiplier;
    } else {
      pkmn[team[i].pkmn.id].playerHp =
        (100 +
          pkmn[team[i].pkmn.id].bst.hp *
            30 *
            Math.pow(1.1, pkmn[team[i].pkmn.id].ivs.hp) *
            (1 + pkmn[team[i].pkmn.id].level * 0.2)) *
        hpMultiplier;
    }
...
function switchMemberNext() {
  //used for stuff like u turn
...
function switchMember(member) {
  ...
  if (pkmn[team[exploreActiveMember].pkmn.id].playerHp > 0) {
    ...
    if (
      document
        ...
        .classList.contains("member-inactive")
    )
      return;
  }
  ...
  for (const learntMoveID of pkmn[team[exploreActiveMember].pkmn.id].movepool)
    if (move[learntMoveID]?.buildup !== undefined)
      move[learntMoveID].buildup = 0;
  ...
  if (testAbility(`active`, ability.naturalCure.id)) {
...
function setPkmnTeam() {
  ...
  document.getElementById(`explore-header`).style.backgroundImage =
    `url(img/bg/${areas[saved.currentArea].background}.png)`;
  ...
  for (const i in team) {
    ...
    if (pkmn[team[i].pkmn.id].shiny)
      pkmnName = `${format(team[i].pkmn.id)} <span style="color:#FF4671;">✦</span> <span class="explore-pkmn-level" id="explore-${i}-lvl">lvl ${pkmn[team[i].pkmn.id].level}</span>`;

    let pkmnSprite = `<img class="sprite-trim" src="img/pkmn/sprite/${team[i].pkmn.id}.png" id="explore-team-member-${i}-sprite">`;
    if (pkmn[team[i].pkmn.id].shiny)
      pkmnSprite = `<img class="sprite-trim" src="img/pkmn/shiny/${team[i].pkmn.id}.png" id="explore-team-member-${i}-sprite">`;
    if (
      pkmn[team[i].pkmn.id].shiny &&
      pkmn[team[i].pkmn.id].shinyDisabled == true
    )
      pkmnSprite = `<img class="sprite-trim" src="img/pkmn/sprite/${team[i].pkmn.id}.png" id="explore-team-member-${i}-sprite">`;
    ...
    setTimeout(() => {
      document.getElementById(
        `explore-team-member-${i}-spriteData`,
      ).dataset.pkmnEditor = pkmn[team[i].pkmn.id].id;
    }, 1);
    ...
    for (const e in pkmn[team[i].pkmn.id].moves) {
      ...
      if (moveId == undefined) {
        ...
        document
          .getElementById(`explore-team-member-${i}-moves`)
          .appendChild(divMove);
        continue;
      }
...
function updatePreviewTeam() {
  ...
  if (saved.tutorial && saved.tutorialStep === "travel") {
    saved.tutorialStep = "moves";
    openTutorial();
  }
  saved.firstTimePlaying = false;
  ...
  for (const i in currentTeam) {
    ...
    if (currentTeam[i].pkmn !== undefined) {
      ...
      let itemDiv = `<svg xmlns="http://www.w3.org/2000/svg" width="256" height="256" viewBox="0 0 256 256"><g fill="currentColor"><path d="M224 128a96 96 0 1 1-96-96a96 96 0 0 1 96 96" opacity="0.2"/><path d="M128 24a104 104 0 1 0 104 104A104.11 104.11 0 0 0 128 24m0 192a88 88 0 1 1 88-88a88.1 88.1 0 0 1-88 88m48-88a8 8 0 0 1-8 8h-32v32a8 8 0 0 1-16 0v-32H88a8 8 0 0 1 0-16h32V88a8 8 0 0 1 16 0v32h32a8 8 0 0 1 8 8"/></g></svg>`;
      if (currentTeam[i].item !== undefined)
        itemDiv = `<img src="img/items/${currentTeam[i].item}.png">`;
      ...
      let pkmnName = `${format(currentTeam[i].pkmn)} ${nameTag} <span class="explore-pkmn-level" id="explore-${i}-lvl">lvl ${pkmn[currentTeam[i].pkmn].level}</span>`;
      if (pkmn[currentTeam[i].pkmn].shiny)
        pkmnName = `${format(currentTeam[i].pkmn)} ${nameTag} <span style="color:#FF4671;">✦</span> <span class="explore-pkmn-level" id="explore-${i}-lvl">lvl ${pkmn[currentTeam[i].pkmn].level}</span>`;

      let pkmnSprite = `<img class="sprite-trim" src="img/pkmn/sprite/${currentTeam[i].pkmn}.png" id="explore-team-member-${i}-sprite">`;
      if (pkmn[currentTeam[i].pkmn].shiny)
        pkmnSprite = `<img class="sprite-trim" src="img/pkmn/shiny/${currentTeam[i].pkmn}.png" id="explore-team-member-${i}-sprite">`;
      if (
        pkmn[currentTeam[i].pkmn].shiny &&
        pkmn[currentTeam[i].pkmn].shinyDisabled == true
      )
        pkmnSprite = `<img class="sprite-trim" src="img/pkmn/sprite/${currentTeam[i].pkmn}.png" id="explore-team-member-${i}-sprite">`;
      ...
      document.getElementById(
        `explore-team-member-${i}-spriteData`,
      ).dataset.pkmnEditor = currentTeam[i].pkmn;

      //change team item by clicking
      document
        .getElementById(`team-${i}-held-item`)
      ...
      for (const e in pkmn[currentTeam[i].pkmn].moves) {
        ...
        divMove.dataset.move = moveId;
        document
          .getElementById(`explore-team-member-${i}-moves-preview`)
          .appendChild(divMove);
      }
    }
...
function handleTouchMove(e) {
  ...
  if (ghostElement) {
    ...
    ghostElement.style.left =
      touch.clientX -
      (ghostElement.className === "team-held-item"
        ? 0
