...
function updateItemShop() {
  document.getElementById("shop-currency").innerHTML =
    `<img src="img/items/bottleCap.png"> x${item.bottleCap.got}`;
  document.getElementById("shop-currency-gold").innerHTML =
    `<img src="img/items/goldenBottleCap.png"> x${item.goldenBottleCap.got}`;
  ...
  for (let i in shop) {
    if (shop[i].category !== shopCategory && shop[i].category !== "all")
      continue;

    if (item[shop[i].icon].type == "held" && item[shop[i].icon].got >= 20)
      continue;
    ...
    function buyItem(amount) {
      if (item[currency].got >= shop[i].price * amount) {
        ...
        document.getElementById("tooltipMid").style.display = "none";
        document.getElementById("tooltipBottom").innerHTML =
          `You dont have enough Bottle Caps to purchase this<span id="prevent-tooltip-exit"></span>`;
      }
    }

    div.dataset.item = shop[i].icon;

    if (item[i] && item[i].type == "tm") {
      div.innerHTML = `
