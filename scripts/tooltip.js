...
function closeTooltip() {
  if (
    document.getElementById("team-name-field") &&
  ...
  if (tooltipStack.length > 0) {
    ...
    voidAnimation(
      "tooltipBackground",
      "tooltipBoxAppear 0.2s reverse 1 ease-in",
    );
    setTimeout(() => {
...
document.getElementById("tooltipBackground").addEventListener("click", (e) => {
  if (
    e.target.id === "tooltipBackground" &&
    !document.getElementById("prevent-tooltip-exit")
  ) {
    closeTooltip();
  }
});
...
function tooltipData(category, ttdata) {
  ...
  if (category == "area") {
    ...
    document.getElementById("tooltipBottom").innerHTML =
      `<div id="area-preview-spawns"><strong>Area Pokemon</strong></div>`;
    ...
    for (const [listName, list] of Object.entries(spawns)) {
      for (const item of list) {
        ...
        if (listName === "uncommon") {
          tag = `<span>Uncommon</span>`;
        }
        if (listName === "rare") {
          tag = `<span>Rare!</span>`;
        }
        ...
        if (
          pkmn[item.id].shiny &&
          areas[ttdata].uncatchable != true &&
          areas[ttdata].type != "dungeon"
        )
          tag += `<div class="wild-shiny-tag">✦</div>`;

        div.className = "area-preview";
        if (
          pkmn[item.id].caught === 0 &&
          areas[ttdata].type !== "dungeon" &&
          areas[ttdata].uncatchable != true
        )
          div.classList.add("hidden-pkmn");
        if (
          pkmn[item.id].caught > 0 ||
          areas[ttdata].type == "dungeon" ||
          areas[ttdata].uncatchable
        )
          div.dataset.pkmn = item.id;

        div.innerHTML =
          `<img class="sprite-trim" src="img/pkmn/sprite/${item.id}.png">` +
          tag;
        if (
          pkmn[item.id].shiny &&
          areas[ttdata].uncatchable != true &&
          ...
          div.innerHTML =
            `<img class="sprite-trim" src="img/pkmn/shiny/${item.id}.png">` +
            tag;
    ...
    if (areas[ttdata].drops !== undefined) {
      document.getElementById("tooltipMid").innerHTML =
        `<div id="area-preview-items"><strong>Area Items</strong></div>`;
      ...
      for (const [listName, list] of Object.entries(drops)) {
        for (const item of list) {
          ...
          if (item.type !== "tm")
            div.innerHTML =
              `<img style="scale:2" src="img/items/${item.id}.png">` + tag;
          if (item.type == "tm")
            div.innerHTML =
              `<img style="scale:2" src="img/items/tm${format(move[item.move].type)}.png">` +
              tag;
  ...
  if (category == "trainer") {
    ...
    document.getElementById("tooltipMid").style.display = "none";
    document.getElementById("tooltipBottom").innerHTML =
      `<div id="area-preview-spawns"><strong>Team Preview</strong></div>`;
    ...
    for (const item of spawns) {
      ...
      if (
        pkmn[item].shiny &&
        areas[ttdata].type != "vs" &&
        areas[ttdata].type != "frontier"
      )
        tag += `<div class="wild-shiny-tag">✦</div>`;
      ...
      if (
        ...
        div.innerHTML = `<img class="sprite-trim" src="img/pkmn/shiny/${item}.png"> ${tag}`;
      document.getElementById("area-preview-spawns").appendChild(div);
    }

    if (areas[ttdata].reward !== undefined) {
      document.getElementById("tooltipMid").innerHTML =
        `<div id="area-preview-items"><strong>Victory Rewards</strong></div>`;
      ...
      for (const i of rewards) {
        ...
        if (item[i.id] != undefined) {
          div.dataset.item = i.id;
          meow = "items";
        } else {
          div.dataset.pkmn = i.id;
        }
        div.className = "area-preview";
        if (i.type !== "tm")
          div.innerHTML = `<img style="scale:2" src="img/${meow}/${i.id}.png">`;
        if (i.type == "tm")
          div.innerHTML =
            `<img style="scale:2" src="img/${meow}/tm${format(move[i.move].type)}.png">` +
            tag;
        if (meow == `pkmn/sprite`)
          div.innerHTML = `<img style="scale:1" src="img/${meow}/${i.id}.png">`;
        document.getElementById("area-preview-items").appendChild(div);
      }
    }
  ...
  if (category == "buff") {
    ...
    if (ttdata === "burn")
      document.getElementById("tooltipBottom").innerHTML =
        `Decreases Physical Attack by 50% and deals damage every turn`;
    if (ttdata === "poisoned")
      document.getElementById("tooltipBottom").innerHTML =
        `Decreases Special Attack by 50% and deals damage every turn`;
    if (ttdata === "sleep")
      document.getElementById("tooltipBottom").innerHTML =
    ...
    if (ttdata === "sunny")
      document.getElementById("tooltipBottom").innerHTML =
        `Increases the damage of Fire-Type moves by 75% and decreases the damage of Water-Type moves by 50%`;
    if (ttdata === "rainy")
      document.getElementById("tooltipBottom").innerHTML =
        `Increases the damage of Water-Type moves by 75% and decreases the damage of Fire-Type moves by 50%`;
    if (ttdata === "sandstorm")
      document.getElementById("tooltipBottom").innerHTML =
  ...
  if (category == "help") {
    ...
    if (ttdata === `Frontier`)
      document.getElementById("tooltipTitle").innerHTML = `Battle Frontier`;
    if (ttdata === `Frontier`)
      document.getElementById("tooltipBottom").innerHTML =
    ...
    if (ttdata === `Spiral`)
      document.getElementById("tooltipTitle").innerHTML = `Spiraling Tower`;
    if (ttdata === `Spiral`)
      document.getElementById("tooltipBottom").innerHTML =
    ...
    if (ttdata === `Spiral`)
      document.getElementById("tooltipMid").innerHTML =
        `Current Type Rotation: ${format(saved.currentSpiralingType)}`;

    if (ttdata === `Wild Areas`)
    ...
    if (ttdata === `Training`)
      document.getElementById("tooltipTitle").innerHTML = `Training`;
    if (ttdata === `Training`)
      document.getElementById("tooltipBottom").innerHTML =
    ...
    if (ttdata === `Genetics`)
      document.getElementById("tooltipTitle").innerHTML = `Genetics`;
    if (ttdata === `Genetics`)
      document.getElementById("tooltipBottom").innerHTML =
    ...
    if (ttdata === `Pokerus`)
      document.getElementById("tooltipTitle").innerHTML = `Pokerus`;
    if (ttdata === `Pokerus`)
      document.getElementById("tooltipBottom").innerHTML =
    ...
    if (ttdata === `settingsNewPkmn`)
      document.getElementById("tooltipTitle").innerHTML = `Setting`;
    if (ttdata === `settingsNewPkmn`)
      document.getElementById("tooltipBottom").innerHTML =
        `Automatically hides got Pokemon that are not new after a battle. Excluded from this setting are: New Pokemon, Iv's Ups and Shiny Pokemon`;

    if (ttdata === `settingsAlternateWild`)
      document.getElementById("tooltipTitle").innerHTML = `Setting`;
    if (ttdata === `settingsAlternateWild`)
      document.getElementById("tooltipBottom").innerHTML =
        `Decreases the current Wild Area rotation by -1. Useful if you missed yesterday's rotation, or if your schedule doesnt line up with my game`;

    if (ttdata === `searchDictionary`)
      document.getElementById("tooltipTitle").innerHTML = `Keywords`;
    if (ttdata === `searchDictionary`)
      document.getElementById("tooltipBottom").innerHTML =
        `Operators:<br>![keyword]: Exclude from search<br>[keywordA] or [keywordB]: Search keywordA OR keywordB<br>[keywordA] [keywordB]: Search for keywordA AND keywordB<br><br>Pokemon keywords:<br>unobtainable, wild, park, event, frontier, shiny, caught, [type], [hidden ability]<br><br>Move keywords:<br>physical, special, [type], [ability]`;
    if (ttdata === `searchDictionary`)
      document.getElementById("dictionary-search").blur();

    if (ttdata === `searchPokedex`)
      document.getElementById("tooltipTitle").innerHTML = `Keywords`;
    if (ttdata === `searchPokedex`)
      document.getElementById("tooltipBottom").innerHTML =
        `Operators:<br>![keyword]: Exclude from search<br>[keywordA] or [keywordB]: Search keywordA OR keywordB<br>[keywordA] [keywordB]: Search for keywordA AND keywordB<br><br>Keywords:<br>shiny, pokerus, [type], [ability], [hidden ability], [level], [move]`;
    if (ttdata === `searchPokedex`)
      document.getElementById("pokedex-search").blur();

    if (ttdata === `Main Challenges`)
      document.getElementById("tooltipTitle").innerHTML = `Main Challenges`;
    if (ttdata === `Main Challenges`)
      document.getElementById("tooltipBottom").innerHTML =
        `Story-driven battles and curated challenges live here. More Main Challenges are coming soon.`;

    if (ttdata === `Custom Challenges`)
      document.getElementById("tooltipTitle").innerHTML = `Custom Challenges`;
    if (ttdata === `Custom Challenges`)
      document.getElementById("tooltipBottom").innerHTML =
  ...
  if (category == "ability") {
    ...
    if (ability[ttdata].rarity === 2)
      document.getElementById("tooltipMid").innerHTML = `Uncommon Ability`;
    if (ability[ttdata].rarity === 3)
      document.getElementById("tooltipMid").innerHTML = `Rare Ability`;
  ...
  if (category == "ribbon") {
    ...
    document.getElementById("tooltipMid").innerHTML =
      ribbon[ttdata].description;
    if (
      ttdata == `tower1` &&
      pkmn[currentEditedPkmn].recordSpiraling != undefined
    )
      document.getElementById("tooltipMid").innerHTML +=
        `<br>(Highest Spiraling Tower floor reached: ${pkmn[currentEditedPkmn].recordSpiraling})`;
    openTooltip();
  }
  ...
  if (category == "move") {
    ...
    document.getElementById("tooltipTitle").style.display = `inline`;
    document.getElementById("tooltipTop").innerHTML =
      `<img src="img/items/tm${format(move[ttdata].type)}.png">`;
    document.getElementById("tooltipTitle").innerHTML = format(ttdata);
    document.getElementById("tooltipMid").style.display = "inline";
    ...
    if (affectedAbilities.length > 0)
      affectedText = `<br>Affected by ${joinWithAnd(affectedAbilities)}`;
    if (move[ttdata].restricted)
      affectedText += `<br>This move is restricted (${restrictedIcon}) and only one of them can be present in the active moves at a time`;

    document.getElementById("tooltipMid").innerHTML =
      `${format(move[ttdata].type)}, ${move[ttdata].power} Power, ${format(move[ttdata].split)}${affectedText}`;
    if (move[ttdata].info == undefined)
      document.getElementById("tooltipBottom").innerHTML =
        `No additional effects`;
    else
      document.getElementById("tooltipBottom").innerHTML = move[ttdata].info();
    openTooltip();
  }

  if (category == "item") {
    document.getElementById("tooltipTop").style.display = "flex";
    if (item[ttdata].type !== "tm")
      document.getElementById("tooltipTop").innerHTML =
        `<img src="img/items/${ttdata}.png">`;
    if (item[ttdata].type == "tm")
      document.getElementById("tooltipTop").innerHTML =
        `<img src="img/items/tm${format(move[item[ttdata].move].type)}.png">`;
  ...
  if (category == "pkmn") {
    ...
    let signatureMove = "";
    if (pkmn[ttdata].signature != undefined)
    ...
    if (pkmn[ttdata].hiddenAbility != undefined)
      hiddenAbility = format(pkmn[ttdata].hiddenAbility.id);

    document.getElementById("tooltipMid").innerHTML =
      `${returnTypeMultipliers(pkmn[ttdata])}`;
    ...
    document.getElementById("tooltipBottom").innerHTML = `
        ...
        `;
    if (pkmn[ttdata].hiddenAbility != undefined)
      document.getElementById("inpect-pkmn-ability").dataset.ability =
        pkmn[ttdata].hiddenAbility.id;
    if (pkmn[ttdata].signature != undefined)
      document.getElementById("inpect-pkmn-signature").dataset.move =
        pkmn[ttdata].signature.id;

    if (pkmn[ttdata].caught > 0) {
      document.getElementById(`inspect-pkmn-image`).dataset.pkmnEditor =
        pkmn[ttdata].id;
    }
  ...
  if (category == "pkmnEditor") {
    ...
    if (
      poke.signature !== undefined &&
      poke.level == 100 &&
      !poke.movepool.includes(poke.signature.id)
    )
      poke.movepool.push(poke.signature.id);

    currentEditedPkmn = poke.id;
    tooltipStack.length = 0;
    closeTooltip();

    if (pkmn[currentEditedPkmn].tag)
    ...
    let nameTag = "";
    if (pkmn[poke.id].pokerus)
      nameTag = `<span data-help="Pokerus"><span style="color:white; background:${returnTypeColor("poison")}; padding:0 0.3rem; border-radius:3px; margin-left:0.3rem; cursor:help;">PKRS</span></span>`;

    if (pkmn[poke.id].shiny && pkmn[poke.id].shinyDisabled != true)
      document.getElementById("pkmn-editor-sprite").src =
        `img/pkmn/shiny/${poke.id}.png`;
    else
      document.getElementById("pkmn-editor-sprite").src =
        `img/pkmn/sprite/${poke.id}.png`;
    document.getElementById("pkmn-editor-name").innerHTML =
      `${format(poke.id)}${nameTag}`;
    if (pkmn[poke.id].shiny)
      document.getElementById("pkmn-editor-name").innerHTML =
        `${format(poke.id)} <span style="color:#FF4671">✦</span>${nameTag}`;
    document.getElementById("pkmn-editor-level").innerHTML =
      `Level ${poke.level}`;
    document.getElementById("pkmn-editor-type").innerHTML = returnPkmnTypes(
      poke.id,
    );
    ...
    if (pkmn[ttdata].evolve && pkmn[ttdata].evolve()[1].level > 0) {
      ...
      if (pkmn[pkmn[ttdata].evolve()[1].pkmn.id].caught > 0)
        evolutionTag = `<span>Unlocks ${format(pkmn[ttdata].evolve()[1].pkmn.id)} at level ${pkmn[ttdata].evolve()[1].level} ✔️</span>`;
    }
    ...
    if (pkmn[ttdata].evolve) {
      for (const evo in pkmn[ttdata]?.evolve()) {
        ...
        if (
          pkmn[ttdata].evolve()[evo].item == undefined ||
          item[pkmn[ttdata].evolve()[evo].item.id] == undefined
        )
          continue;
        let levelRequired = wildAreaLevel2;
        if (pkmn[ttdata].evolve()[evo].item.id == "linkStone")
          levelRequired = wildAreaLevel4;
        if (pkmn[ttdata].evolve()[evo].item.id == "oddRock")
          levelRequired = wildAreaLevel4;
        if (pkmn[ttdata].evolve()[evo].pkmn.id.slice(0, 4) === "mega")
          levelRequired = 100;
        if (pkmn[pkmn[ttdata].evolve()[evo].pkmn.id].caught > 0)
          evolutionTag += `<span>Unlocks ${format(pkmn[ttdata].evolve()[evo].pkmn.id)} by using a ${format(item[pkmn[ttdata].evolve()[evo].item.id].id)} at level ${levelRequired}+ ✔️</span>`;
        else
          evolutionTag += `<span>Unlocks ${format(pkmn[ttdata].evolve()[evo].pkmn.id)} by using a ${format(item[pkmn[ttdata].evolve()[evo].item.id].id)} at level ${levelRequired}+ ❌</span>`;
      }
    }
    ...
    document.getElementById("pkmn-edit-division").innerHTML =
      `<span>${returnDivisionLetter(returnPkmnDivision(poke))}</span>division`;

    if (poke.type.includes(`dragon`))
      document.getElementById(
        `pkmn-editor-side-content`,
      ).style.backgroundImage = `url(img/bg/mini/special3.png)`;
    if (poke.type.includes(`steel`))
      document.getElementById(
        `pkmn-editor-side-content`,
      ).style.backgroundImage = `url(img/bg/mini/city.png)`;
    if (poke.type.includes(`electric`))
      document.getElementById(
        `pkmn-editor-side-content`,
      ).style.backgroundImage = `url(img/bg/mini/city2.png)`;
    if (poke.type.includes(`ghost`))
      document.getElementById(
        `pkmn-editor-side-content`,
      ).style.backgroundImage = `url(img/bg/mini/cave.png)`;
    if (poke.type.includes(`normal`))
      document.getElementById(
        `pkmn-editor-side-content`,
      ).style.backgroundImage = `url(img/bg/mini/city2.png)`;
    if (poke.type.includes(`fighting`))
      document.getElementById(
        `pkmn-editor-side-content`,
      ).style.backgroundImage = `url(img/bg/mini/city.png)`;
    if (poke.type.includes(`bug`))
      document.getElementById(
        `pkmn-editor-side-content`,
      ).style.backgroundImage = `url(img/bg/mini/forest.png)`;
    if (poke.type.includes(`dark`))
      document.getElementById(
        `pkmn-editor-side-content`,
      ).style.backgroundImage = `url(img/bg/mini/cavern.png)`;
    if (poke.type.includes(`poison`))
      document.getElementById(
        `pkmn-editor-side-content`,
      ).style.backgroundImage = `url(img/bg/mini/cave.png)`;
    if (poke.type.includes(`fairy`))
      document.getElementById(
        `pkmn-editor-side-content`,
      ).style.backgroundImage = `url(img/bg/mini/pink.png)`;
    if (poke.type.includes(`ground`))
      document.getElementById(
        `pkmn-editor-side-content`,
      ).style.backgroundImage = `url(img/bg/mini/beach.png)`;
    if (poke.type.includes(`flying`))
      document.getElementById(
        `pkmn-editor-side-content`,
      ).style.backgroundImage = `url(img/bg/mini/sky.png)`;
    if (poke.type.includes(`rock`))
      document.getElementById(
        `pkmn-editor-side-content`,
      ).style.backgroundImage = `url(img/bg/mini/desert.png)`;
    if (poke.type.includes(`water`))
      document.getElementById(
        `pkmn-editor-side-content`,
      ).style.backgroundImage = `url(img/bg/mini/ocean.png)`;
    if (poke.type.includes(`ice`))
      document.getElementById(
        `pkmn-editor-side-content`,
      ).style.backgroundImage = `url(img/bg/mini/mountain.png)`;
    if (poke.type.includes(`grass`))
      document.getElementById(
        `pkmn-editor-side-content`,
      ).style.backgroundImage = `url(img/bg/mini/forest.png)`;
    if (poke.type.includes(`fire`))
      document.getElementById(
        `pkmn-editor-side-content`,
      ).style.backgroundImage = `url(img/bg/mini/volcano.png)`;
    if (poke.type.includes(`psychic`))
      document.getElementById(
        `pkmn-editor-side-content`,
      ).style.backgroundImage = `url(img/bg/mini/space.png)`;

    if (pkmn[ttdata].ability == undefined)
      pkmn[ttdata].ability = learnPkmnAbility(pkmn[ttdata].id);
    ...
    if (ability[pkmn[ttdata].ability].rarity == 2)
      document
        .getElementById("pkmn-edit-ability")
        .classList.add("ability-uncommon");
    if (ability[pkmn[ttdata].ability].rarity == 3)
      document
        .getElementById("pkmn-edit-ability")
        .classList.add("ability-rare");
    document.getElementById("pkmn-edit-ability").dataset.ability =
      pkmn[ttdata].ability;
    document.getElementById("pkmn-edit-ability").innerHTML =
      `<span>${abilityIcon + format(pkmn[ttdata].ability)}</span>${abilityTier}`;
    ...
    if (pkmn[ttdata].signature) {
      ...
      document.getElementById("pkmn-stats-stat-signature").style.display =
        "flex";
      document.getElementById("pkmn-edit-signature").innerHTML =
        `${signatureIcon} ${format(pkmn[ttdata].signature.id)}`;
      if (pkmn[ttdata].movepool.includes(pkmn[ttdata].signature.id))
        document.getElementById("pkmn-edit-signature").innerHTML =
          `${signatureIcon} ${format(pkmn[ttdata].signature.id)} ✔️`;
      document.getElementById("pkmn-edit-signature").dataset.move =
        pkmn[ttdata].signature.id;
    } else
      document.getElementById("pkmn-stats-stat-signature").style.display =
        "none";
    ...
    if (pkmn[ttdata].hiddenAbility) {
      ...
      if (ability[pkmn[ttdata].hiddenAbility.id].rarity == 2)
        abilityHiddenTier = ` <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M21.97 14.607a1.07 1.07 0 0 1-.73 1l-1.88.62a3.9 3.9 0 0 0-1.56 1a4.06 4.06 0 0 0-1 1.57l-.65 1.87a1.14 1.14 0 0 1-.38.52a1.1 1.1 0 0 1-.63.2a1 1 0 0 1-.62-.2a1.07 1.07 0 0 1-.39-.53l-.63-1.88a4 4 0 0 0-2.53-2.54l-1.88-.62a1.13 1.13 0 0 1-.53-.39a1.06 1.06 0 0 1 .54-1.64l1.87-.62a4 4 0 0 0 2.56-2.55l.62-1.86a1 1 0 0 1 .36-.52a1 1 0 0 1 .61-.23a1 1 0 0 1 .64.18a1 1 0 0 1 .41.52l.63 1.9a4 4 0 0 0 2.55 2.56l1.87.65a1 1 0 0 1 .52.38a1.1 1.1 0 0 1 .23.61M12.1 7.656a1 1 0 0 1-.67.93l-1.34.44a2.6 2.6 0 0 0-1 .64a2.7 2.7 0 0 0-.64 1l-.47 1.34a1 1 0 0 1-.34.47a1.05 1.05 0 0 1-.58.19a1 1 0 0 1-.93-.68l-.44-1.34a2.6 2.6 0 0 0-.64-1a2.7 2.7 0 0 0-1-.64l-1.35-.45a.92.92 0 0 1-.48-.36a.93.93 0 0 1-.19-.57a1 1 0 0 1 .19-.58a1 1 0 0 1 .49-.34l1.34-.45a2.7 2.7 0 0 0 1-.64c.29-.277.509-.62.64-1l.45-1.32a1 1 0 0 1 .33-.48a.93.93 0 0 1 .56-.2a.87.87 0 0 1 .58.16a1 1 0 0 1 .38.47l.45 1.37c.135.378.354.72.64 1a2.7 2.7 0 0 0 1 .64l1.35.47a1 1 0 0 1 .65.92z"/></svg>`;
      if (ability[pkmn[ttdata].hiddenAbility.id].rarity == 3)
        abilityHiddenTier = ` <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M21.738 16.13a1 1 0 0 1-.19.61a1 1 0 0 1-.52.38l-1.71.57a3.6 3.6 0 0 0-1.4.86a3.5 3.5 0 0 0-.86 1.4l-.6 1.7a1 1 0 0 1-.36.51a1.1 1.1 0 0 1-.62.19a1 1 0 0 1-1-.71l-.57-1.71a3.5 3.5 0 0 0-.86-1.4a3.8 3.8 0 0 0-1.4-.87l-1.71-.56a1.1 1.1 0 0 1-.51-.37a1.1 1.1 0 0 1-.21-.62a1 1 0 0 1 .71-1l1.72-.57a3.54 3.54 0 0 0 2.28-2.28l.57-1.69a1 1 0 0 1 .95-.73c.215 0 .426.059.61.17c.182.125.322.303.4.51l.58 1.74a3.54 3.54 0 0 0 2.28 2.28l1.7.6a1 1 0 0 1 .51.38a1 1 0 0 1 .21.61m-9.999-6.36a1 1 0 0 1-.17.55a1 1 0 0 1-.47.35l-1.26.42c-.353.122-.673.32-.94.58a2.5 2.5 0 0 0-.58.94l-.43 1.24a.9.9 0 0 1-.35.47a1 1 0 0 1-.56.18a1 1 0 0 1-.57-.19a1 1 0 0 1-.34-.47l-.41-1.25a2.44 2.44 0 0 0-.58-.93a2.2 2.2 0 0 0-.93-.58l-1.25-.42a.93.93 0 0 1-.48-.35a1 1 0 0 1 .48-1.47l1.25-.41a2.49 2.49 0 0 0 1.53-1.53l.41-1.23a1 1 0 0 1 .32-.47a1 1 0 0 1 .55-.2a1 1 0 0 1 .57.16a1 1 0 0 1 .37.46l.42 1.28a2.49 2.49 0 0 0 1.53 1.53l1.25.43a.92.92 0 0 1 .46.35a.94.94 0 0 1 .18.56m5.789-5.36a1 1 0 0 1-.17.51a.82.82 0 0 1-.42.3l-.62.21a.84.84 0 0 0-.52.52l-.22.63a.93.93 0 0 1-.29.39a.82.82 0 0 1-.52.18a1.1 1.1 0 0 1-.49-.15a.9.9 0 0 1-.32-.44l-.21-.62a.7.7 0 0 0-.2-.32a.76.76 0 0 0-.32-.2l-.62-.2a1 1 0 0 1-.42-.31a.9.9 0 0 1-.16-.51a.94.94 0 0 1 .17-.51a.9.9 0 0 1 .42-.3l.61-.2a.9.9 0 0 0 .33-.2a.9.9 0 0 0 .2-.33l.21-.62c.06-.155.155-.292.28-.4a1 1 0 0 1 .49-.19a.94.94 0 0 1 .53.16a1 1 0 0 1 .32.41l.21.64a.9.9 0 0 0 .2.33a1 1 0 0 0 .32.2l.63.21a1 1 0 0 1 .41.3a.87.87 0 0 1 .17.51"/></svg>`;
      document.getElementById("pkmn-edit-ability-hidden").className = "";
      if (ability[pkmn[ttdata].hiddenAbility.id].rarity == 2)
        document
          .getElementById("pkmn-edit-ability-hidden")
          .classList.add("ability-uncommon");
      if (ability[pkmn[ttdata].hiddenAbility.id].rarity == 3)
        document
          .getElementById("pkmn-edit-ability-hidden")
          .classList.add("ability-rare");
      if (pkmn[ttdata].hiddenAbilityUnlocked != true)
        abilityHiddenIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M2 16c0-2.828 0-4.243.879-5.121C3.757 10 5.172 10 8 10h8c2.828 0 4.243 0 5.121.879C22 11.757 22 13.172 22 16s0 4.243-.879 5.121C20.243 22 18.828 22 16 22H8c-2.828 0-4.243 0-5.121-.879C2 20.243 2 18.828 2 16" opacity="0.5"/><path fill="currentColor" d="M6.75 8a5.25 5.25 0 0 1 10.5 0v2.004c.567.005 1.064.018 1.5.05V8a6.75 6.75 0 0 0-13.5 0v2.055a24 24 0 0 1 1.5-.051z"/></svg>`;
      if (pkmn[ttdata].hiddenAbilityUnlocked != true) {
        document.getElementById("pkmn-edit-ability-hidden").style.filter =
          "brightness(0.5)";
      } else {
        document.getElementById("pkmn-edit-ability-hidden").style.filter =
          "brightness(1)";
      }
      document.getElementById("pkmn-edit-ability-hidden").dataset.ability =
        pkmn[ttdata].hiddenAbility.id;
      document.getElementById("pkmn-edit-ability-hidden").innerHTML =
        `<span>${abilityHiddenIcon + format(pkmn[ttdata].hiddenAbility.id)}</span>${abilityHiddenTier}`;
    } else {
      document.getElementById("pkmn-edit-ability-hidden").className = "";
      document.getElementById("pkmn-edit-ability-hidden").innerHTML =
        `<span>${abilityHiddenIcon}WIP</span>`;
      if (document.getElementById("pkmn-edit-ability-hidden").dataset.ability)
        delete document.getElementById("pkmn-edit-ability-hidden").dataset
          .ability;
    }
    ...
    function updateMoves() {
      for (const key in pkmn[ttdata].moves) {
        ...
        if (pkmn[ttdata].moves.slot1 == undefined)
          pkmn[ttdata].moves.slot1 = undefined;
        if (pkmn[ttdata].moves.slot2 == undefined)
          pkmn[ttdata].moves.slot2 = undefined;
        if (pkmn[ttdata].moves.slot3 == undefined)
          pkmn[ttdata].moves.slot3 = undefined;
        if (pkmn[ttdata].moves.slot4 == undefined)
          pkmn[ttdata].moves.slot4 = undefined;
        ...
        let signatureIcon = "";
        if (move[moveId].moveset == undefined)
          signatureIcon = `<svg style="color:${returnTypeColor(move[moveId].type)}" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M21.951 9.67a1 1 0 0 0-.807-.68l-5.699-.828l-2.548-5.164A.98.98 0 0 0 12 2.486v16.28l5.097 2.679a1 1 0 0 0 1.451-1.054l-.973-5.676l4.123-4.02a1 1 0 0 0 .253-1.025" opacity="0.5"/><path fill="currentColor" d="M11.103 2.998L8.555 8.162l-5.699.828a1 1 0 0 0-.554 1.706l4.123 4.019l-.973 5.676a1 1 0 0 0 1.45 1.054L12 18.765V2.503a1.03 1.03 0 0 0-.897.495"/></svg>`;
        if (move[moveId].restricted)
          signatureIcon += `<svg style="color:${returnTypeColor(move[moveId].type)}" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M12.832 21.801c3.126-.626 7.168-2.875 7.168-8.69c0-5.291-3.873-8.815-6.658-10.434c-.619-.36-1.342.113-1.342.828v1.828c0 1.442-.606 4.074-2.29 5.169c-.86.559-1.79-.278-1.894-1.298l-.086-.838c-.1-.974-1.092-1.565-1.87-.971C4.461 8.46 3 10.33 3 13.11C3 20.221 8.289 22 10.933 22q.232 0 .484-.015c.446-.056 0 .099 1.415-.185" opacity="0.5"/><path fill="currentColor" d="M8 18.444c0 2.62 2.111 3.43 3.417 3.542c.446-.056 0 .099 1.415-.185C13.871 21.434 15 20.492 15 18.444c0-1.297-.819-2.098-1.46-2.473c-.196-.115-.424.03-.441.256c-.056.718-.746 1.29-1.215.744c-.415-.482-.59-1.187-.59-1.638v-.59c0-.354-.357-.59-.663-.408C9.495 15.008 8 16.395 8 18.445"/></svg>`;
    ...
    class="pkmn-movebox-progress" style="background: ${returnTypeColor(move[moveId].type)} "></div><span>` +
        ...
        document
          .getElementById(`pkmn-editor-current-moves`)
          .appendChild(divMove);
      }
    ...
    function updateMovepool() {
      ...
      for (const e of sortedMovepool) {
        ...
        if (
          pkmn[ttdata].moves.slot1 == undefined &&
          pkmn[ttdata].moves.slot1 != e &&
          pkmn[ttdata].moves.slot2 != e &&
          pkmn[ttdata].moves.slot3 != e &&
          pkmn[ttdata].moves.slot4 != e
        )
          pkmn[ttdata].moves.slot1 = e;
        if (
          pkmn[ttdata].moves.slot2 == undefined &&
        ...
        divMove.addEventListener("click", (e) => {
          ...
          if (
            ...
            document.getElementById("tooltipTitle").innerHTML =
              `Restricted Action`;
            document.getElementById("tooltipMid").innerHTML =
              `Moves cannot be freely switched at this moment`;
            openTooltip();
            return;
          }
  ...
  if (category == "dictionaryAbility") {
    ...
    document.getElementById("tooltipTitle").innerHTML = format(ttdata);
    document.getElementById("tooltipMid").innerHTML =
      `Common Ability<br>${ability[ttdata].info()}`;
    if (ability[ttdata].rarity === 2)
      document.getElementById("tooltipMid").innerHTML =
        `Uncommon Ability<br>${ability[ttdata].info()}`;
    if (ability[ttdata].rarity === 3)
      document.getElementById("tooltipMid").innerHTML =
        `Rare Ability<br>${ability[ttdata].info()}`;
    ...
    if (ability[ttdata].type) {
      ...
      if (signatureArray.length > 0)
        signatureText = `<br><br>Additionally, appears as the Hidden ability of ${joinWithAnd(signatureArray)}`;
  ...
  if (category == "dictionaryPkmn") {
    ...
    document.getElementById("tooltipTitle").style.display = `none`;
    document.getElementById("tooltipTop").innerHTML =
      `<div style="cursor:help" data-pkmn="${ttdata}"><img src="img/pkmn/sprite/${ttdata}.png"></div>`;
    if (pkmn[ttdata].shiny)
      document.getElementById("tooltipTop").innerHTML =
        `<div style="cursor:help" data-pkmn="${ttdata}"><img src="img/pkmn/shiny/${ttdata}.png"></div>`;
    if (pkmn[ttdata].caught == 0)
      document.getElementById("tooltipTop").innerHTML =
        `<div><img style="filter:brightness(0)" src="img/pkmn/sprite/${ttdata}.png"></div>`;
    document.getElementById("tooltipTitle").innerHTML = format(ttdata);
    document.getElementById("tooltipMid").style.display = "inline";
    ...
    for (const i in areas) {
      if (areas[i].type == "wild") {
        if (
          areas[i].spawns.common.includes(pkmn[ttdata]) ||
          areas[i].spawns.uncommon.includes(pkmn[ttdata]) ||
          areas[i].spawns.rare.includes(pkmn[ttdata])
        ) {
      ...
      if (areas[i].encounter) {
        if (
          areas[i].team.slot1 == pkmn[ttdata] ||
          areas[i].reward.includes(pkmn[ttdata])
        )
          encounterSpawn = areas[i].id;
      }
    }
    ...
    let spawnLocation = "";
    if (wildSpawn != "")
      spawnLocation += `<span>Found in the wild area ${format(wildSpawn)} (Rotation ${areas[wildSpawn].rotation})</span>`;
    if (
      wildlifePoolCommon.includes(ttdata) ||
      wildlifePoolUncommon.includes(ttdata) ||
      wildlifePoolRare.includes(ttdata)
    )
      spawnLocation += `<span>Found randomly in the Wildlife Park wild area</span>`;
    if (exclusiveFrontierPkmn.includes(pkmn[ttdata]))
      spawnLocation += `<span>Obtained as a random reward in the Battle Frontier</span>`;
    if (encounterSpawn != "")
      spawnLocation += `<span>Obtained in the event ${format(areas[encounterSpawn].name)} (Rotation ${areas[encounterSpawn].rotation})</span>`;
    if (eventSpawn != "")
      spawnLocation += `<span>Found in the event ${format(eventSpawn)} (Rotation ${areas[eventSpawn].rotation})</span>`;
    if (spawnLocation == "")
      spawnLocation = `This Pokemon cannot be caught on its current stage`;
    if (pkmn[ttdata].tagObtainedIn == "unobtainable")
      spawnLocation = `This Pokemon is unobtainable`;
  ...
  if (category == "dictionaryItem") {
    ...
    document.getElementById("tooltipMid").innerHTML =
      `${item[ttdata].info()}<br>(You have ${item[ttdata].got})`;
    ...
    for (const i in areas) {
      if (areas[i].type == "wild") {
        if (
          areas[i].drops?.common?.includes(item[ttdata]) ||
          areas[i].drops?.uncommon?.includes(item[ttdata]) ||
          areas[i].drops?.rare?.includes(item[ttdata])
        ) {
    ...
    if (
      ...
      obtainText += `<span>Obtained via Export Reward</span>`;

    for (const i in spiralingRewards)
      if (spiralingRewards[i].item == ttdata)
        obtainText += `<span>Reward from Spiraling Tower</span>`;

    for (const i in shop) {
      if (shop[i].icon == ttdata)
        obtainText += `<span>Can be bought in the Poke-Mart</span>`;
    }

    if (obtainText == "")
      obtainText += `<span>This item is currently unobtainable</span>`;

    document.getElementById("tooltipBottom").innerHTML =
      `<span style="display:flex; flex-direction:column">${obtainText}<span>`;
...
document.addEventListener("contextmenu", (e) => {
  ...
  if (el.parentElement && el !== el.parentElement) {
    if (el.parentElement.contains(el) && el.children.length === 0) {
      el = el.parentElement;
    }
  }
