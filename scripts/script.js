...
document
  .getElementById("settings-alternate-rotation")
  .addEventListener("change", (e) => {
    saved.alternateWildRotation = document.getElementById(
...
function learnPkmnMove(id, level, mod, exclude = []) {
  ...
  while (attempts++ < MAX_ATTEMPTS) {
    ...
    if (level === 1) {
      if (!typeMatch.length) continue;
      const chosenMove =
        typeMatch[Math.floor(Math.random() * typeMatch.length)];
      if (move[chosenMove].power <= 0) continue;
      return move[chosenMove].id;
    }

    let chosenList;
    if (rng(0.7) && typeMatch.length) {
      chosenList = typeMatch;
    } else if (rng(0.5) && movesetMatch.length) {
...
function openTutorial() {
  ...
  if (saved.tutorialStep == "battleEnd") {
    document.getElementById("tutorial-text").innerHTML =
...
guide.inspecting = {
  ...
  description: function () {
    return `Right click/long press on most elements can give further information. You can further right click/long press on information within information.<br><br>Some elements that can be inspected include areas, trainers, moves, status effects, wild pokemon, team pokemon and items`;
  },
};
...
guide.stab = {
  name: `Battle: STAB`,
  description: function () {
    return `If a Pokemon uses a damaging move that has the same type as one of that Pokemon's types, the move's damage is increased by x1.5<br>This is known as same-type attack bonus, or STAB`;
  },
};
...
guide.battleFatigue = {
  ...
  description: function () {
    return `Pokemon lose a very small fraction of their maximum health when attacking. This damage can be mitigated by a sum of the HP, Defense and Special Defense of the Pokemon, meaning bulkier Pokemon can battle for longer`;
  },
};
...
guide.weather = {
  name: `Battle: Weather`,
  description: function () {
    return `Certain moves can change the weather to altered ones such as ${tagSunny}, ${tagRainy}, ${tagSandstorm}, ${tagHail}, ${tagFoggy}, ${tagElectricTerrain}, ${tagGrassyTerrain} or ${tagMistyTerrain}<br><br>You can further check their effects by right click/long press<br><br>Altered weathers will last for 15 turns, and only can be changed after 30`;
  },
};
...
guide.compatibility = {
  ...
  description: function () {
    return `Compatibility determines how similar the sample is to the host. This influences various parameters such as the chances of inherit, or shiny mutations (only if the sample is shiny)<br><br>Sharing one type with the sample will yield one level of compatibility, whereas sharing two types will increase it by two levels.<br><br>Additionally, if the sample is of the same evolutive line as the host, it will recieve maximum compatibility`;
  },
};
...
guide.powerCost = {
  ...
  description: function () {
    return `Power cost determines how taxing it is to modify the host. This influences the time to complete the operation<br><br>This is determined by the division of the host. A higher division will exponentially increase the time required to modify the Pokemon`;
  },
};
...
function setGuide() {
  for (const i in guide) {
    ...
    div.addEventListener("click", (e) => {
      ...
      document.getElementById("tooltipBottom").innerHTML =
        `<span style="overflow-y:scroll; max-height:25rem; display:inline-block;">${guide[i].description()}</span>`;
...
function infoPkmn() {
  console.table([
    ...
    {
      command: "pkmn.NAME.ivs.hp=NUMBER",
      effect: "Modify Pokemon ivs (hp, atk, satk, def, sdef, spe)",
    },
    {
      command: "pkmn.NAME.movepool.push(move.NAME.id)",
      effect: "Add Pokemon Move",
    },
    {
      command: "pkmn.NAME.ability=ability.NAME.id",
      effect: "Modify Pokemon Ability",
    },
    {
      command: "pkmn.NAME.hiddenAbilityUnlocked=true",
      effect: "Unlock Hidden Ability",
    },
  ]);
}

function infoItem() {
  console.table([{ command: "item.NAME.got=AMOUNT", effect: "Give Items" }]);
}
...
function infoRotation() {
  console.table([
    ...
    {
      command: "rotationDungeonCurrent=NUMBER",
      effect: "Modify Dungeon Rotation",
    },
    { command: "rotationEventCurrent=NUMBER", effect: "Modify Event Rotation" },
    {
      command: "rotationFrontierCurrent=NUMBER",
      effect: "Modify Frontier Rotation",
    },
  ]);
}
...
function infoMisc() {
  console.table([
    {
      command: "saved.overrideBattleTimer=NUMBER",
      effect: "Alter Battle Speed (Default 2000)",
    },
    {
      command: "debugGetPkmn(LEVEL,'shiny')",
      effect: "Get all Pokemon at certain level. Shiny is optional",
    },
    {
      command: "debugSetIvs(NUMBER)",
      effect: "Set all Pokemon IV's. Maximum 6",
    },
    {
      command: "saved.geneticOperation=1",
      effect: "Complete Genetics Operation",
    },
    {
      command: "getMoveset(pkmn.NAME,LEVEL)",
      effect: "Generates a table of possible Pokemon moves",
    },
  ]);
}
